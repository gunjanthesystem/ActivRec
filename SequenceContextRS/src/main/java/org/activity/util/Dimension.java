...
public class Dimension implements Serializable
{
	...
	Dimension(String dimensionName, String dimensionPrimaryKeyValue)
	{
		...
		try
		{
			...
			// ArrayList of Attributes of this dimension. Each ArrayList element is an ArrayList of attribute's parameters. The attribute parameters are 'attribute name','datatype' and 'hierarchy' in that order.
			/*
			 * example of dimensionAttributes, (User_Age, Int, h1), (User_Name, String, h2) , .....
			 */
			ArrayList<ArrayList<String>> dimensionAttributeInfos = ConnectDatabase.getDimensionAttributes(dimensionName);
			// System.out.println("dimensionAttributeInfos size "+dimensionAttributeInfos.size());//@toremoveatruntime
			// LOGGSystem.out.println("Got attributes for dimension="+dimensionName+" no. of attributes="+dimensionAttributeInfos.size());
			...
			for (int i = 0; i < dimensionAttributeInfos.size(); i++)
			{
				...
				ArrayList<String> dimensionAttributeInfo = dimensionAttributeInfos.get(i); // example (User_Name, String, h2)
			...
			ResultSet resultSet = ConnectDatabase.getSQLResultSet(" * ", dimensionName.toLowerCase() + "_table ", dimensionPrimaryKey + " = " + dimensionPrimaryKeyValue);// "User_ID = "+userID);
			
			// while(resultSet.next()) // the while loops only once, we are using loops to check that there are not more than one rows
			// {
			...
			int countCheck = 0; // If all is correct then, the result set should contain only one row as we are selecting by a primary key value
			...
			while (resultSet.next()) // the while loops only once, we are using loops to check that there are not more than one rows
			{
				...
				for (Map.Entry<String, String> entry : attributeNameDatatypePairs.entrySet())
				{
					...
					{
						String attributeValue = resultSet.getString(entry.getKey()); // example: resultSet.getString("Use_Name")
					...
					else if (datatype.equalsIgnoreCase("Date"))
					{
						...
						// System.out.println("Date encountered: value ="+attributeValue.toString());////@toremoveatruntime
						attributeNameValuePairs.put(entry.getKey(), attributeValue);
					}
					...
					else if (datatype.equalsIgnoreCase("Time"))
					{
						...
						// System.out.println("Time encountered: value ="+attributeValue.toString());////@toremoveatruntime
						attributeNameValuePairs.put(entry.getKey(), attributeValue);
					}
				}
			...
			if (countCheck > 1)
			{
				System.err.println("Error in Dimension constructor: more than one rows returned from database for dimension=" + dimensionName + " for a give primary key value");
			}
	...
	{
		// System.out.println("Value of dimension attribute "+attributeName+" is "+this.attributeNameValuePairs.get(attributeName));
