...
public class SAXTrieTree
{
	...
	public SAXTrieTree(Integer alphabetSize) throws TrieException
	{
		...
		{
			throw new TrieException("Unable to create trie structure for the alphabet size of " + alphabetSize);
		}
	}
	 ...
	 * @throws TrieException
	 *             If error occurs - wrong string length provided etc.
	 */
	public List<Integer> getOccurrences(String string) throws TrieException
	{
		...
		// sanity check
		//
		if (this.root.getDescendants().size() > 0 && string.length() > 0 && this.alphabetSize == string.length()
				&& !containsWrongSymbols(string))
		{
	...
	public List<Integer> addOccurence(String str, int idx) throws TrieException
	{
		...
		if ((null == str) || (str.length() != this.alphabetSize) || (containsWrongSymbols(str)))
		{
			throw new TrieException(
					"Cannot populate occurrence of \"" + str + "\"into the trie of size " + this.alphabetSize);
		}
	 ...
	 *
	 * @param root
	 *            The current node, IT IS NOT ROOT NODE, we just call it root here due to context.
	 * @param str
	 *            The string to use for traversal from this node.
	 * @return list of found occurrences.
	 * @throws TrieException
	 *             If error occurs.
	 */
	private List<Integer> getSubstringOccurrence(TrieAbstractNode root, String str) throws TrieException
	{
