...
public class GridCell implements Comparable<GridCell>
{
	 ...
	 * <li>This first two digits consist of the resolution incremented by 22 in case of negative latitude, by 44 in case
	 * of negative longitude, and by 66 in case of negative latitude and longitude. In case that the latitude/longitude
	 * is strictly less than .5e-6, or that the difference of longitude to 180 or -180 degrees is strictly less than
	 * .5e-6, the respective sign is always regarded as being positive.</li>
	 * <li>The consecutive digits consist of the latitude, with two pre-decimal and six decimal places.</li>
	 * <li>The consecutive digits consist of the longitude, with three pre-decimal and six decimal places. The longitude
	 * is per definition 0 if the latitude differs from -90 or 90 degrees by strictly less than .5e-6. The longitude is
	 * expected to be greater than -180 and strictly less than 180 degrees.</li>
	 * </ul>
	 *
	 * If the grid cell type ADAPTIVE_UNIQUE is provided, the values for latitude and longitude are only encoded with
	 * the precision required to guarantee uniqueness of the IDs. In case of ADAPTIVE_1_PERCENT, the values for latitude
	 * and longitude are encoded with two more decimal places as in the previous case.
	 *
	...
	public Long getID(GridCellIDType gridCellIDType)
	{
		...
		if (id == null)
		{
			int numberOfDecimalPlaces = GridCellMetaData.getInstance().numberOfDecimalPlaces(this.getResolution(),
					gridCellIDType);
			double precisionPerDefinition = .5 * Math.pow(10, -numberOfDecimalPlaces);
			long sgnLat = (this._lat <= -precisionPerDefinition) ? 22 : 0;
			long sgnLon = (this._lon <= -precisionPerDefinition && 180 - Math.abs(this._lon) >= precisionPerDefinition)
					? 44
					: 0;
			id = (this._isPentagon ? -1 : 1) * ((this._resolution.longValue() + sgnLat + sgnLon)
					* (long) Math.pow(10, 2 * numberOfDecimalPlaces + 5)
					+ Math.abs(Math.round((this._lat + this._precision) * Math.pow(10, numberOfDecimalPlaces)))
							* (long) Math.pow(10, numberOfDecimalPlaces + 3)
					+ Math.abs(Math.round((this._lon + this._precision) * Math.pow(10, numberOfDecimalPlaces))));
