...
public class generateSyntheticData
{
	public static void main(String[] args)
	{
		...
		for (int userId = 0; userId <= 6; userId++) // 7 Users
		{
			...
			int userAge = randomInRange(20, 35);
			...
			for (int month = 3; month <= 4; month++)
			{
				...
				for (int day = 1; day <= numberOfDaysInMonth; day++)// 30 days
				{
					...
					// int numberOfActivities=randomInRange(4,7);// a user performs 3 to 5 activities per day. Min + (int)(Math.random() * ((Max - Min) + 1))
					
					int startInDayHour = randomInRange(5, 8);
					int startInDayMinutes = randomInRange(0, 59);
					int previousEndHour = startInDayHour;
					int previousEndMinute = startInDayMinutes;
					int previousActivityId = -1; // to make sure that consecutive activities are not the same. (this will create an effect of merging similar consecutive activities)
					...
					for (int activityIterator = -1;/* activityIterator<=numberOfActivities && (previousEndHour+4 <23) */; activityIterator++)
					{
						...
						if (activityIterator == -1) // This is the activity before the listed activities start in the day, this will be an 'other' activity
						{
						...
						else if (!(previousEndHour + 4 < 23) || activityIterator == 7) // other activity after the listed activities end in the day
						{
						...
						// listed activity
						{
							activityId = randomInRange(0, 14); // 15 Activities
							...
							// int randomDurationInMinutes = randomInRange(15,getActivityMaxDurationInMinutes(activityId));
							...
							endHour = startHour + randomInRange(1, 4);
							endMinute = randomInRange(0, 59);
						...
						LocationObject locationObject = LocationObject.getSyntheticLocationObjectInstance(userId, activityId);
						
						timeId = Integer.parseInt(Integer.toString(startHour) + Integer.toString(startMinute) + Integer.toString(endHour) + Integer.toString(endMinute));
						
						String timeCategory = getTimeCategory(startHour);
						Timestamp startTimestamp = new Timestamp(year - 1900, month - 1, day, startHour, startMinute, 0, 0);// Timestamp(int year, int month, int date, int hour, int minute, int
																															// second, int nano)
						String weekDay = "default", weekOfYear = "default";
						try
						{
							weekDay = ConnectDatabase.getSQLStringResultSingleColumn("SELECT DAYNAME('" + startTimestamp + "');").get(0);
							weekOfYear = ConnectDatabase.getSQLStringResultSingleColumn("SELECT WEEKOFYEAR('" + startTimestamp + "');").get(0);
						}
	...
	public static String getActivityCategory(int activityId)
	{
		...
		/*
		 * switch(activityId%3) { case 0: activityCategory="category 1"; break; //commuting, computer, on phone, taking care, eating, case 1: activityCategory="category 2"; break; //working,
		 * watchingtv. relaxing, preparing food, nap/resting case 2: activityCategory="category 3"; break; //socialising, shopping, meditating, housework, exercising default:
		 * activityCategory="others"; break; }
		 */
