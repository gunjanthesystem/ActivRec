...
public class CombinedLevelsEditDistance extends AlignmentBasedDistance
{
	...
	{
		super();
		System.out.println("Error: Using CombinedLevelEditDistance, we are not supposed to use this");// this error message is dependent on our current experimental setup
	}
	 ...
	 * @return
	 */
	public final Pair<String, Double> getEditDistanceWithoutEndCurrentActivity(ArrayList<ActivityObject> activityObjects1Original, ArrayList<ActivityObject> activityObjects2Original)
	{
		...
		// $$WritingToFile.writeEditSimilarityCalculation(activityObjects1, activityObjects2, result.getSecond(), result.getFirst());
	 ...
	 * @return
	 */
	public final double getEditDistance(ArrayList<ActivityObject> activityObjects1, ArrayList<ActivityObject> activityObjects2)
	{
		...
		{
			System.err.println("Error: In getEditSimilarity: cost for insertion, deletion and replacement not set properly");
		}
		
		if ((costInsertActivityObject != costDeleteActivityObject))
		{
			System.err
					.println("Warning: In getEditSimilarity: cost for insertion and deletion are different. Are you sure about this? (because insertion from the persective of one object is deletion from the perspective of the other");
		}
		// to understand the programming logic: refer: http://people.cs.pitt.edu/~kirk/cs1501/Pruhs/Spring2006/assignments/editdistance/Levenshtein%20Distance.htm and
		...
		{
			distMatrix = new double[len1 + 1][len2 + 1][3]; // the third dimension stores count of deletion, countInsertion,countReplacments
			...
			for (int i = 0; i < len1; i++)
			{
				...
				// String location1=ae1.getLocationName();//ae1.getDimensionAttributeValue("Location_Dimension", "Location_Name").toString();
				// String startTime1=ae1.getStartTimestamp().toString();//ae1.getDimensionAttributeValue("Time_Dimension", "Start_Time").toString();
				String startTime1 = ae1.getStartTimestamp().getHours() + ":" + ae1.getStartTimestamp().getMinutes() + ":" + ae1.getStartTimestamp().getSeconds();
				...
				for (int j = 0; j < len2; j++)
				{
					...
					// String location2=ae2.getLocationName();//getDimensionAttributeValue("Location_Dimension", "Location_Name").toString();
					// String startTime2=ae2.getStartTimestamp().toString();//getDimensionAttributeValue("Time_Dimension", "Start_Time").toString();
					String startTime2 = ae2.getStartTimestamp().getHours() + ":" + ae2.getStartTimestamp().getMinutes() + ":" + ae2.getStartTimestamp().getSeconds();
					long duration2 = ae2.getDurationInSeconds();
					
					// System.out.println("\nComparing Activity Name 1="+activityName1+" -- "+"Activity Name 2="+activityName2);
					/*
					 * System.out.println("Location 1="+location1+" -- "+"Location 2="+location2); System.out.println("Start Time=1"+startTime1+" -- "+"Start Time2="+startTime2);
					 * System.out.println("Duration 1="+duration1+" -- "+"Duration 2="+duration2);
					 */
					
					double insertCost = 0;// = distMatrix[i][j + 1] + costInsertActivityObject; // value from the cell above // whether this insertion or deletion depends on the which object's
											// perspective we are saying,
											// so the wt of insertion and deletion should be same
					double deleteCost = 0;// = distMatrix[i + 1][j] + costDeleteActivityObject; // value from the cell on the left.
					...
					{
						// replace += this.costReplaceEqualActivityObject; //=0, dummy because usually the replacement wt when objects are equal will be 0.
						// replaceCost=0;
					...
					else
					{
						...
						{
							/*
							 * if(location1.equals(location2) ==false) { replace += this.costReplaceLocation;// System.out.print("\n\t~~~location different for "+activityName1+" "+activityName2);
							...
							if (startTime1.equals(startTime2) == false)
							{
								...
								// replace += this.costReplaceStartTime;//System.out.print("\n\t~~~start time different for "+activityName1+" "+activityName2);
							...
							if (duration1 != duration2)
							{
								...
								// replace += this.costReplaceDuration;//System.out.print("\n\t~~~duration different for"+activityName1+" "+activityName2);
						...
						{
							replaceCost = costReplaceActivityObject;// System.out.println("$$$$$$ adding wtReplaceActivityObject");
						}
						
						//
						
						// ///
						double costOfReplacing = distMatrix[i][j][0] + distMatrix[i][j][1] + distMatrix[i][j][2] + replaceCost;
						double costOfInserting = distMatrix[i + 1][j][0] + distMatrix[i + 1][j][1] + distMatrix[i + 1][j][2] + costInsertActivityObject;
						double costOfDeleting = distMatrix[i][j + 1][0] + distMatrix[i][j + 1][1] + distMatrix[i][j + 1][2] + costDeleteActivityObject;
						
						// System.out.print("cost: DIS="+costOfDeleting+" "+costOfInserting+" "+replaceCost);
						
						if (isMinimum(costOfDeleting, costOfDeleting, costOfInserting, costOfReplacing))// (costOfInserting < costOfReplacing && costOfInserting<costOfDeleting)
						{
						...
						else if (isMinimum(costOfInserting, costOfDeleting, costOfInserting, costOfReplacing))// if(costOfInserting < costOfReplacing && costOfInserting<costOfDeleting)
						{
						...
						// if (isMinimum(costOfReplacing,costOfDeleting,costOfInserting,costOfReplacing))//else //(costOfReplacing < costOfInserting && costOfReplacing<costOfDeleting)
						{
		...
		// similarity=100- ((double)editDistance/(Math.max(activityObjects1.size(), activityObjects2.size()) ))*100;//userDayActivitiesAsStringCode.subSequence(0,
	 ...
	 * @return
	 */
	public final double getEditDistanceInvalidsExpunged(ArrayList<ActivityObject> activityObjects1, ArrayList<ActivityObject> activityObjects2)
	{
	 ...
	 * @return
	 */
	public final Pair<String, Double> getEditDistanceWithoutEndCurrentActivityInvalidsExpunged(ArrayList<ActivityObject> activityObjects1, ArrayList<ActivityObject> activityObjects2)
	{
		return getEditDistanceWithoutEndCurrentActivity(expungeInvalids(activityObjects1), expungeInvalids(activityObjects2));// similarity;
	}
	 ...
	 * @return
	 */
	public final Pair<String, Double> getEditDistanceWithTrace(ArrayList<ActivityObject> activityObjects1, ArrayList<ActivityObject> activityObjects2)
	{
		// double similarity=0d;//,distance=0d;
		String trace = "";
		double editDistance = 0d;
			...
			for (int i = 0; i < len1; i++)
			{
				...
				for (int j = 0; j < len2; j++)
				{
					...
					else
					{
						...
						if (isMinimum(costOfDeleting, costOfDeleting, costOfInserting, costOfReplacing))// (costOfDeleting < costOfReplacing && costOfDeleting<costOfInserting)
						{
							distMatrix[i + 1][j + 1][0] = distMatrix[i][j + 1][0] + costDeleteActivityObject;// costOfDeleting;
							distMatrix[i + 1][j + 1][1] = distMatrix[i][j + 1][1] + 0;
							distMatrix[i + 1][j + 1][2] = distMatrix[i][j + 1][2] + 0;
						...
						// if (isMinimum(costOfReplacing,costOfDeleting,costOfInserting,costOfReplacing))//else //(costOfReplacing < costOfInserting && costOfReplacing<costOfDeleting)
						{
							...
							if (replaceCost == costReplaceActivityObject)
								...
								replaceCode = "Sd";
							else if (replaceCost == (wtDuration + wtStartTime))
								replaceCode = "Sstd";
							traceMatrix[i + 1][j + 1].append(traceMatrix[i][j] + "_" + replaceCode + "(" + (i + 1) + "-" + (j + 1) + ")");
							// System.out.println(" minimum cost is for replacing");
							// numberOfReplacements++;
						}
					}
	 ...
	 * @return
	 */
	public final Pair<String, Double> getMyLevenshteinDistanceAsStringCodeWithTrace(ArrayList<ActivityObject> activityObjects1, ArrayList<ActivityObject> activityObjects2)
	{
		activityObjects1 = pruneFirstUnknown(activityObjects1);
		activityObjects2 = pruneFirstUnknown(activityObjects2);
		...
		// double levenshteinDistance =StringUtils.getLevenshteinDistance(stringCodeForActivityObjects1, stringCodeForActivityObjects2);
		Pair<String, Double> levenshteinDistance = getMySimpleLevenshteinDistance(stringCodeForActivityObjects1, stringCodeForActivityObjects2, 1, 1, 1);
		
		// $$WritingToFile.writeEditSimilarityCalculation(activityObjects1, activityObjects2, levenshteinDistance.getSecond(), levenshteinDistance.getFirst());
 ...
 * 
 * // for simplicity we will give equal wts to all dimension atttributes ArrayList<Dimension> dimensionsOf1= activityObject1.getDimensions(); ArrayList<Dimension> dimensionsOf2=
 * activityObject2.getDimensions();
 * 
 * for(int i=0;i<dimensionsOf1.size();i++) { //since each activity's dimensions are stored in same order and each activity has same number of dimensions HashMap<String,Object>
 * attributeNameValuePairs1=dimensionsOf1.get(i).getAttributeNameValuePairs(); //name value pairs of that dimension, say 'User Dimension' HashMap<String,Object>
 * attributeNameValuePairs2=dimensionsOf2.get(i).getAttributeNameValuePairs(); //name value pairs of that dimension, say 'User Dimension' } return 0; }
 */
...
// * @return
// */
// public final double getCaseBasedV1Similarity(ActivityObject activityObject1, ActivityObject activityObject2, int userID)
// {
// int ui = getIndexOfUserID(userID);
// double startTimeComponent = 0, durationComponent = 0, distanceTravelledComponent = 0, startGeoComponent = 0, endGeoComponent = 0, avgAltComponent = 0;
//
// // /////////////////////////////////////////////////////////////////////
// double startTimeAct1InSecs = activityObject1.getStartTimestamp().getHours() * 60 * 60 + (activityObject1.getStartTimestamp().getMinutes()) * 60 +
// (activityObject1.getStartTimestamp().getSeconds());
// double startTimeAct2InSecs = activityObject2.getStartTimestamp().getHours() * 60 * 60 + (activityObject2.getStartTimestamp().getMinutes()) * 60 +
...
// // ///////////////////////////////////////////////////////
//
// double absDifferenceOfDuration = Math.abs(activityObject1.getDurationInSeconds() - activityObject2.getDurationInSeconds());
// double maxOfDurationInSeconds = Math.max(activityObject1.getDurationInSeconds(), activityObject2.getDurationInSeconds());
//
...
// double maxDistanceTravelled = Math.max(activityObject1.getDistanceTravelled(), activityObject2.getDistanceTravelled());
//
...
// // ///////////////////////////////////////////////////////////////////////////////////////////
//
// double diffStartGeo = UtilityBelt.haversine(activityObject1.getStartLatitude(), activityObject1.getStartLongitude(), activityObject2.getStartLatitude(), activityObject2.getStartLongitude());
//
// startGeoComponent = 0;
//
...
// double dist = 0;
//
// if (diffStartGeo > iqr3StartGeo) // higher than third quartile..then it is considered to have no similarity, that is distance =1
// {
// dist = 1;
// }
// else
...
// // ///////////////////////////////////////////////////////////////////////////////////////////
//
// double diffEndGeo = UtilityBelt.haversine(activityObject1.getEndLatitude(), activityObject1.getEndLongitude(), activityObject2.getEndLatitude(), activityObject2.getEndLongitude());
//
// endGeoComponent = 0;
//
...
// double dist = 0;
//
// if (diffEndGeo > iqr3endGeo) // higher than third quartile..then it is considered to have no similarity, that is distance =1
// {
// dist = 1;
// }
// else
...
// maxAvgAltitude = Math.max(Double.parseDouble(activityObject1.getAvgAltitude()), Double.parseDouble(activityObject2.getAvgAltitude()));
//
// if (maxAvgAltitude <= 0)
// {
// maxAvgAltitude = 1;
// if (maxAvgAltitude < 0)
// {
// System.err.println("Error in getCaseBasedV1Similarity: maxAvgAltitude=" + maxAvgAltitude + "<0" + "Does that data contain places below sea level?");
// }
// }
//
// avgAltComponent = 0;
...
// // ////////////////////////////////////////////////////////////////////////////////////////////////
//
// double result = // costReplaceStartTime * (1-absDifferenceOfStartTime/maxStartTimeInSeconds) + costReplaceDuration * (1- absDifferenceOfDuration/maxOfDurationInSeconds)
// costReplaceStartTime * startTimeComponent + costReplaceDuration * durationComponent + costReplaceDistanceTravelled * distanceTravelledComponent + costReplaceStartGeo * startGeoComponent
// + costReplaceEndGeo * endGeoComponent + costReplaceAvgAltitude * avgAltComponent;
// ;
//
// System.out.println("Inside getCaseBasedV1Similarity \n ActivityObject1:" + activityObject1.getActivityName() + "__" + activityObject1.getStartTimestamp() + "__" +
// activityObject1.getDurationInSeconds()
// + "__" + activityObject1.getDistanceTravelled() + "__" + activityObject1.getStartLatitude() + "," + activityObject1.getStartLongitude() + "__" + activityObject1.getEndLatitude() + ","
// + activityObject1.getEndLongitude() + "__" + activityObject1.getAvgAltitude()
//
// + "\n ActivityObject2:" + activityObject2.getActivityName() + "__" + activityObject2.getStartTimestamp() + "__" + activityObject2.getDurationInSeconds() + "__"
// + activityObject2.getDistanceTravelled() + "__" + activityObject2.getStartLatitude() + "," + activityObject2.getStartLongitude() + "__" + activityObject2.getEndLatitude() + ","
// + activityObject2.getEndLongitude() + "__" + activityObject2.getAvgAltitude());
//
// System.out.println("CaseSimilarity =" + result + " = " + costReplaceStartTime + " * (1-" + absDifferenceOfStartTime + "/" + maxStartTimeInSeconds + ") " + "+ " + costReplaceDuration + " * (1- "
// + absDifferenceOfDuration + "/" + maxOfDurationInSeconds + ")" + "+ " + costReplaceDistanceTravelled + " * (1- " + absDiffDistance + "/" + maxDistanceTravelled + ")" + "+" + costReplaceStartGeo
// + " * " + startGeoComponent + "+" + costReplaceEndGeo + " * " + endGeoComponent + "+ " + costReplaceAvgAltitude + " * (1- " + absDiffAvgAlt + "/" + maxAvgAltitude + ")");
//
// if (result < 0)
// {
...
// public final double getLevenshteinDistanceAsStringCode(ArrayList<ActivityObject> activityObjects1, ArrayList<ActivityObject> activityObjects2)
// {
// activityObjects1=pruneFirstUnknown(activityObjects1);
// activityObjects2=pruneFirstUnknown(activityObjects2);
//
