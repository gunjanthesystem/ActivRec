...
public class FeatureWiseEditDistance extends AlignmentBasedDistance
{
	...
	/**
	 * Returns the feature wise edit distance using ONLY SINGLE FEATURE. (which feature to use is set by parameters in Class Constant)
	 * 
	 * @param activityObjects1Original
	 * @param activityObjects2Original
	 * @return
	 */
	public Pair<String, Double> getFeatureWiseEditDistanceWithTraceSingleFeature(ArrayList<ActivityObject> activityObjects1Original,
			ArrayList<ActivityObject> activityObjects2Original)
	{
		if (Constant.verboseDistance)
		{
			System.out.println("Debug note: calc (single) feature-wise editDist between " + activityObjects1Original.size() + " & "
		...
		LinkedHashMap<String, Pair<String, Double>> resultsFetched =
				getFeatureWiseEditDistanceWithTrace(activityObjects1Original, activityObjects2Original);
		
		if (resultsFetched.size() > 1)
		{
			String msg =
					"Error in org.activity.distances.FeatureWiseEditDistance.getFeatureWiseEditDistanceWithTraceSingleFeature(): More than one features are being used (check features to consider in Constant)";
	 ...
	 * @return ArrayList of Pairs where each Pair object contains the levenshtein distance for that feature and the trace of operations
	 */
			...
			ArrayList<ActivityObject> activityObjects1Original, ArrayList<ActivityObject> activityObjects2Original)
	{
		...
		if (Constant.considerAllFeaturesForFeatureWiseEditDistance || Constant.considerActivityNameInFeatureWiseEditDistance)
		{
			...
			Pair<String, Double> levenshteinActivityName =
					getMySimpleLevenshteinDistance(stringCodeForActivityNames1, stringCodeForActivityNames2, 1, 1, 2);
		...
		if (Constant.considerAllFeaturesForFeatureWiseEditDistance || Constant.considerStartTimeInFeatureWiseEditDistance)
		{
			String stringCodeForStartTimes[] = StringCode.getStringCodesForStartTime(activityObjects1, activityObjects2);
			// String stringCodeForStartTime2 = StringCode.getStringCodeForStartTime(activityObjects2);
			Pair<String, Double> levenshteinStartTime =
					getMySimpleLevenshteinDistance(stringCodeForStartTimes[0], stringCodeForStartTimes[1], 1, 1, 2);
			mapOfDistances.put("StartTime", levenshteinStartTime);
		}
		
		if (Constant.considerAllFeaturesForFeatureWiseEditDistance || Constant.considerDurationInFeatureWiseEditDistance)
		{
			...
			Pair<String, Double> levenshteinDuration =
					getMySimpleLevenshteinDistance(stringCodeForDurations[0], stringCodeForDurations[1], 1, 1, 2);
		...
		{
			if (Constant.considerAllFeaturesForFeatureWiseEditDistance || Constant.considerDistanceTravelledInFeatureWiseEditDistance)
			{
				// String stringCodeForDistanceTravelled1 = StringCode.getStringCodeForDistanceTravelled(activityObjects1);
				String stringCodesForDistanceTravelled[] =
						StringCode.getStringCodesForDistanceTravelled(activityObjects1, activityObjects2);
			...
			if (Constant.considerAllFeaturesForFeatureWiseEditDistance || Constant.considerStartGeoCoordinatesInFeatureWiseEditDistance)
			{
				String stringCodesForStartGeoCoordinates[] =
						StringCode.getStringCodesForStartGeoCoordinates(activityObjects1, activityObjects2);
				Pair<String, Double> levenshteinStartGeoCordinates =
						getMySimpleLevenshteinDistance(stringCodesForStartGeoCoordinates[0], stringCodesForStartGeoCoordinates[1], 1, 1, 2);
				mapOfDistances.put("StartGeoCordinates", levenshteinStartGeoCordinates);
			}
			
			if (Constant.considerAllFeaturesForFeatureWiseEditDistance || Constant.considerEndGeoCoordinatesInFeatureWiseEditDistance)
			{
				String stringCodesForEndGeoCoordinates[] =
						StringCode.getStringCodesForEndGeoCoordinates(activityObjects1, activityObjects2);
			...
			if (Constant.considerAllFeaturesForFeatureWiseEditDistance || Constant.considerAvgAltitudeInFeatureWiseEditDistance)
			{
				...
				Pair<String, Double> levenshteinAvgAltitude =
						getMySimpleLevenshteinDistance(stringCodesForAvgAltitude[0], stringCodesForAvgAltitude[1], 1, 1, 2);
	...
	public final LinkedHashMap<String, Pair<String, Double>> getFeatureWiseEditDistanceWithoutEndCurrentActivity(
			ArrayList<ActivityObject> activityObjects1Original, ArrayList<ActivityObject> activityObjects2Original)// , String userAtRecomm, String dateAtRecomm, String
																													// timeAtRecomm, Integer candidateTimelineId)
	{
		...
		LinkedHashMap<String, Pair<String, Double>> result = getFeatureWiseEditDistanceWithTrace(activityObjects1, activityObjects2);// , userAtRecomm, dateAtRecomm, timeAtRecomm,
																																		// candidateTimelineId);
	...
	{
		return getFeatureWiseEditDistanceWithTrace(expungeInvalids(activityObjects1), expungeInvalids(activityObjects2));// , userAtRecomm, dateAtRecomm, timeAtRecomm,
																															// candidateTimelineId);//
																															// similarity;
	}
	...
	{
		return getFeatureWiseEditDistanceWithoutEndCurrentActivity(expungeInvalids(activityObjects1), expungeInvalids(activityObjects2));
	}
	// /////////
}
