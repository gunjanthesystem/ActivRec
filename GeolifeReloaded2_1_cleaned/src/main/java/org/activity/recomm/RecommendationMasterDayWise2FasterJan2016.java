...
// LinkedHashMap<Date, Triple<Integer, String, Double>> editDistancesMap = getEditDistancesForCandidateTimelines(
...
// }
//
// if (!nextActivityJustAfterRecommPointIsInvalid) // the next activity just after recomm point is valid,i.e.,
// // there exists a valid actual activity
// // happening after current activity in current timeline. Thus,
// // the current activity is to be removed
...
// return new Triple(rankedRecommendedActivityNamesWithRankScoresString,
// rankedRecommendedActivityNamesWithoutRankScoresString, recommendedActivityNamesWithRankScores);
//
// }
//
...
// .getNextValidActivityAfterActivityAtThisPosition(endPointIndexOfCand);
//
// nextActivityNamesFromCandsByEditDistString = nextActivityNamesFromCandsByEditDistString + "__"
...
// Triple score = getDistanceScoreModifiedEdit(entry.getValue(), activitiesGuidingRecomm, userAtRecomm,
// dateAtRecomm, timeAtRecomm, candTimelineID);
//
...
// for (int i = 0; i < indicesOfValids.size() - 1; i++) // skip the last valid because there is no valid activity
// // to recommend after that.
// {
