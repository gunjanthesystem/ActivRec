...
public class RecommendationMasterDayWise2FasterJan2016
{
	 ...
	 * Current Timeline
	 */
	private ArrayList<ActivityObject> activitiesGuidingRecomm; // (activitiesGuidingRecomm) sequence of activity events happening one or before the recomm point on that day
	...
	public RecommendationMasterDayWise2FasterJan2016(LinkedHashMap<Date, UserDayTimeline> trainingTimelines,
			...
			double thresholdVal, String typeOfThreshold)
	{
		...
		this.activitiesGuidingRecomm = currentDayTimeline.getActivityObjectsStartingOnBeforeTime(timestampPointAtRecomm);
		...
		LinkedHashMap<Date, Triple<Integer, String, Double>> editDistancesMap = getEditDistancesForCandidateTimelines(candidateTimelines,
				activitiesGuidingRecomm, this.userIDAtRecomm, this.dateAtRecomm.toString(), this.timeAtRecomm.toString());
		
		// ########Sanity check
		if (editDistancesMap.size() != candidateTimelines.size())
		{
		...
		editDistancesSortedMap =
				(LinkedHashMap<Date, Triple<Integer, String, Double>>) UtilityBelt.sortTripleByThirdValueAscending6(editDistancesMap);
		
		// this.nextActivityNamesFromCandsByEditDistance = getNextActivityNamesFromCandsByEditDistances(activitiesGuidingRecomm, editDistancesSortedMap,
		// trainingTimelines);
		Pair<String, LinkedHashMap<Date, String>> nextActsPairRes =
				getNextActivityNamesFromCandsByEditDistance(activitiesGuidingRecomm, editDistancesSortedMap, trainingTimelines);
		...
		// this.nextActivityNamesFromCandsByEditDist = getNextActivityNamesFromCandsByEditDistance(activitiesGuidingRecomm, editDistancesSortedMap,
		// trainingTimelines);
		...
		// ########Sanity check
		String sanity11[] = nextActivityNamesFromCandsByEditDistString.split(Pattern.quote("__"));
		...
		{
			Triple<String, String, LinkedHashMap<String, Double>> recommRes =
					createRankedTopRecommendation(this.nextActivityNamesFromCandsByEditDistString);
		...
		int indexOfRecommPointInDayTimeline = currentDayTimeline.getIndexOfActivityObjectsAtTime(timestampPointAtRecomm);
		
		if (indexOfRecommPointInDayTimeline + 1 > currentDayTimeline.getActivityObjectsInDay().size() - 1)
		{
			System.err.println("Error in Recommendation Master: No activity event after recomm point in this day timeline");
		}
		
		else
		{
			ActivityObject activityJustAfterRecommPoint =
					currentDayTimeline.getActivityObjectAtPosition(indexOfRecommPointInDayTimeline + 1);
		...
		if (!nextActivityJustAfterRecommPointIsInvalid) // the next activity just after recomm point is valid,i.e., there exists a valid actual activity
														// happening after current activity in current timeline. Thus, the current activity is to be removed
		...
		WritingToFile.writeDistanceScoresSortedMap(this.userAtRecomm, this.dateAtRecomm, this.timeAtRecomm, this.editDistancesSortedMap,
				this.candidateTimelines, this.nextActivityNamesFromCandsByEditDistMap, this.activitiesGuidingRecomm);
	 ...
	 * @return Triple(rankedRecommendedActivityNamesWithRankScoresString, rankedRecommendedActivityNamesWithoutRankScoresString, recommendedActivityNamesWithRankScores)
	 */
	public Triple<String, String, LinkedHashMap<String, Double>>
			createRankedTopRecommendation(String nextActivityNamesFromCandsByWithEditDist)
	{
		...
		String[] splitted1 = nextActivityNamesFromCandsByWithEditDist.split("__");
		int numOfNextActNames = splitted1.length - 1;
		
		LinkedHashMap<String, Double> recommendedActivityNamesWithRankScores = new LinkedHashMap<String, Double>();
		...
		System.out.println("Debug inside createRankedTopRecommendation: numOfNextActNames=splitted1.length-1=" + numOfNextActNames);
		...
		{
			String[] nextActivityNameStringSplitted = splitted1[i].split(Pattern.quote(":"));
		...
		rankedRecommendedActivityNamesWithRankScoresString =
				convertRankedRecommendationWithRankScoresAsString(recommendedActivityNamesWithRankScores);
		rankedRecommendedActivityNamesWithoutRankScoresString =
				convertRankedRecommendationWithoutRankScoresAsString(recommendedActivityNamesWithRankScores);
		System.out.println(
				"Debug inside createRankedTopRecommendation: topRankedString=  " + rankedRecommendedActivityNamesWithRankScoresString);
		...
		return new Triple(rankedRecommendedActivityNamesWithRankScoresString, rankedRecommendedActivityNamesWithoutRankScoresString,
				recommendedActivityNamesWithRankScores);
	...
	// // I can use this instead of this method: this.recommendedActivityNamesWithRankScores.remove(activityAtRecommPoint.getActivityName());
	// }
	//
	
	public String convertRankedRecommendationWithRankScoresAsString(LinkedHashMap<String, Double> recommendationRankscorePairs)
	{
	...
	public String convertRankedRecommendationWithoutRankScoresAsString(LinkedHashMap<String, Double> recommendationRankscorePairs)
	{
		String rankedRecommendationWithoutRankScores = new String();
		for (Map.Entry<String, Double> entry : recommendationRankscorePairs.entrySet())
		{
	...
	public String getTopRecommendedActivityNamesWithoutDistanceString()// old name getTopRecommendedActivitiesWithoutDistance()
	{
		String result = "";
	...
	/**
	 * NOT USED: Return the next activity names for each candidate with the decreasing order of similarity to the current timeline, i.e., increasing order of distance. (NOTE: may
	 * be we can do away with this method in future as this and the next method below is similar except return type, not sure) Removed as the next method can do the same thing if
	 * returns pair
	 * 
	...
	public Pair<String, LinkedHashMap<Date, String>> getNextActivityNamesFromCandsByEditDistance(
			ArrayList<ActivityObject> activitiesGuidingRecomm, LinkedHashMap<Date, Triple<Integer, String, Double>> distanceScoresSorted,
			LinkedHashMap<Date, UserDayTimeline> dayTimelinesForUser)
	{
		...
		for (Map.Entry<Date, Triple<Integer, String, Double>> distEntryOfCand : distanceScoresSorted.entrySet())
		{
			...
			nextActivityNamesFromCandsByEditDistString =
					nextActivityNamesFromCandsByEditDistString + "__" + nextValidAO.getActivityName() + ":" + distanceOfCandTimeline;
			nextActivityNamesFromCandsByEditDistMap.put(dateOfCandTimeline, nextValidAO.getActivityName() + ":" + distanceOfCandTimeline);
		...
		return new Pair(nextActivityNamesFromCandsByEditDistString, nextActivityNamesFromCandsByEditDistMap);
	}
	...
	{
		String unknownAsCode = StringCode.getStringCodeFromActivityName(Constant.INVALID_ACTIVITY1);
		String othersAsCode = StringCode.getStringCodeFromActivityName(Constant.INVALID_ACTIVITY2);
	...
	/**
	 * Returns the edit distance for each candidate timeline. For multiple occurrence of the current activity in candidate timeline, the least distance from current timeline is
	 * considered
	 * 
	 ...
	 * @param dateAtRecomm
	 * @param timeAtRecomm
	 * @return (Cand TimelineID, (End point index of least distant subsequence, String containing the trace of edit operations performed, edit distance of least distant
	 *         subsequence))
	 */
	public LinkedHashMap<Date, Triple<Integer, String, Double>> getEditDistancesForCandidateTimelines(
			LinkedHashMap<Date, UserDayTimeline> candidateTimelines, ArrayList<ActivityObject> activitiesGuidingRecomm, String userAtRecomm,
			String dateAtRecomm, String timeAtRecomm)
	{
		// <Date of CandidateTimeline, (End point index of least distant subsequence, String containing the trace of edit operations performed, edit distance of
		...
		for (Map.Entry<Date, UserDayTimeline> entry : candidateTimelines.entrySet())
		{
			...
			Long candTimelineID = entry.getKey().getTime();// used as dummy, not exactly useful right now
			Triple score = getDistanceScoreModifiedEdit(entry.getValue(), activitiesGuidingRecomm, userAtRecomm, dateAtRecomm, timeAtRecomm,
					candTimelineID);
	...
	public Triple<Integer, String, Double> getDistanceScoreModifiedEdit(UserDayTimeline userDayTimeline,
			ArrayList<ActivityObject> activitiesGuidingRecomm, String userAtRecomm, String dateAtRecomm, String timeAtRecomm,
			Long candidateID)
	{
		...
		for (Map.Entry<Integer, Pair<String, Double>> entry1 : distanceScoresForEachSubsequence.entrySet()) // we only consider the most similar timeline . i.e.
																											// the first entry in this Map
		{
