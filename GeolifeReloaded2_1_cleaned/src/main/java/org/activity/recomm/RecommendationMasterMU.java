...
/**
 * ALERT!: DO NOT USER THRESHOLDING WITH THIS CLASS since here the edit distances are normalised before thredholding, however, they should have been normalised after thresholding
 * as was the case in iiWAS version of the code This generates recommendations using the matching-unit (MU) approach. Timelines are not viewed as Day Timeline but rather as
 * continuous timelines. The matching-unit can be specified as 'hours' or 'counts'.
 * 
 * Timelines are sets of Activity Objects, (and they do not necessarily belong to one day.) (the matching unit used here HJ version) In this version we calculate edit distance of
 * the full candidate and not the least distant subcandidate
 * 
...
{
	// LinkedHashMap<String,LinkedHashMap<Date,UserDayTimeline>> userTimelines; // LinkedHashMap<Date,UserDayTimeline> dayTimelinesForUser;
	private double matchingUnitInCountsOrHours;
	
	private Timeline trainingTimeline;
	private Timeline testTimeline;
	private LinkedHashMap<Integer, TimelineWithNext> candidateTimelines; // here key is the TimelineID, which is already a class variable of Value, So we could
	 ...
	 * Current Timeline
	 */
	private ArrayList<ActivityObject> activitiesGuidingRecomm; // Current Timeline , sequence of activity objects happening from the recomm point back until the
																// matching unit
	...
	/**
	 * List of of top next activity objects with their edit distances and the timeline id of the candidate producing them
	 */
	private ArrayList<Triple<ActivityObject, Double, Integer>> topNextActivityObjects;
	
	/**
	 * (Cand TimelineID, Pair<Trace,Edit distance>) this LinkedHashMap is sorted by the value of edit distance in ascending order
	 */
	private LinkedHashMap<Integer, Pair<String, Double>> editDistancesSortedMapFullCand; // for full candidate scenario
	
	/**
	 * This is only relevant when case type is 'CaseBasedV1' (Cand TimeineId, Edit distance of the end point activity object of this candidate timeline with the current activity
	 * object(activity at recomm point))
	 */
	private LinkedHashMap<Integer, Double> similarityOfEndPointActivityObjectCand; // currently its a similarity and not edit distance
	...
	public final String lookPastType; // public int totalNumberOfProbableCands; // public int numCandsRejectedDueToNoCurrentActivityAtNonLast; // public int
	...
	public int initialiseDistancesUsed()
	{
		...
		switch (dname)
		{
			...
			default:
				PopUps.showError("Error in org.activity.recomm.RecommendationMasterMU.initialiseDistanceUsed(): Unknown distance specified:"
						+ dname);
				System.err
						.println("Error in org.activity.recomm.RecommendationMasterMU.initialiseDistanceUsed(): Unknown distance specified:"
								+ dname);
				// throw new Exception("Error in org.activity.util.Constant.setDistanceUsed(String): Unknown distance specified:" + dname);
				System.exit(-1);
		}
		return 0;
	}
	...
	/**
	 * Recommendation for a particular RT (FOR PERFORANCE, REMOVED ALL LOGGING PRINTS) (NOT INTENDED TO BE USED FOR EXPERIMENTS DUE TO NO LOGGING)
	 * 
	...
	public RecommendationMasterMU(LinkedHashMap<Date, UserDayTimeline> trainingTimelines,
			...
			double thresholdVal, String typeOfThreshold, double matchingUnitInCountsOrHours, String caseType, String lookPastType,
	...
	)
	{
		...
		this.trainingTimeline = new Timeline(trainingTimelines); // converting day timelines into continuous timelines suitable to be used for matching unit
																	// views
		...
		if (Constant.EXPUNGE_INVALIDS_B4_RECOMM_PROCESS)
		{
			...
			System.out.println("Expunging invalids before recommendation process: expunging from test and training timelines");
		}
		
		// //////////////////
		if (lookPastType.equalsIgnoreCase("Count"))
		{
			this.currentTimeline = TimelineWithNext.getCurrentTimelineFromLongerTimelineMUCount(testTimeline, this.dateAtRecomm,
					this.timeAtRecomm, this.userIDAtRecomm, this.matchingUnitInCountsOrHours);
		}
		
		else if (lookPastType.equalsIgnoreCase("Hrs"))
		{
			this.currentTimeline = TimelineWithNext.getCurrentTimelineFromLongerTimelineMUHours(testTimeline, this.dateAtRecomm,
					this.timeAtRecomm, this.userIDAtRecomm, this.matchingUnitInCountsOrHours);
		}
		else
		{
		...
		// $$LOGGINGSystem.out.println("Current timeline: " + currentTimeline.getActivityObjectNamesWithTimestampsInSequence() + "="
		...
		// $$LOGGINGSystem.out.println("\nDebug note192: getActivityNamesGuidingRecommwithTimestamps() " + getActivityNamesGuidingRecommwithTimestamps()
		// $$LOGGING + "\n size of current timeline=" + currentTimeline.getActivityObjectsInTimeline().size());
		
		this.activityAtRecommPoint = activitiesGuidingRecomm.get(activitiesGuidingRecomm.size() - 1); // current ActivityObject
		...
		// System.out.println("\nDebug note192_223: getActivityNamesGuidingRecommwithTimestamps() " + getActivityNamesGuidingRecommwithTimestamps() +
		...
		/*
		 * Old getHJEditDistancesForCandidateTimelinesFullCand(candidateTimelines, activitiesGuidingRecomm, caseType, this.userIDAtRecomm, this.dateAtRecomm.toString(),
		 * this.timeAtRecomm.toString());
		 */
		
		// getDistanceScoresforCandidateTimelines(candidateTimelines,activitiesGuidingRecomm);
		// getNormalisedDistancesForCandidateTimelinesFullCand(candidateTimelines, activitiesGuidingRecomm, caseType, this.userIDAtRecomm,
		// this.dateAtRecomm.toString(),
		// this.timeAtRecomm.toString(), Constant.getDistanceUsed());// getDistanceScoresforCandidateTimelines(candidateTimelines,activitiesGuidingRecomm);
		
		// //////////////////////////////////
		// System.out.println("\nDebug note192_229: getActivityNamesGuidingRecommwithTimestamps() " + getActivityNamesGuidingRecommwithTimestamps() +
		// " size of current timeline="
		// + currentTimeline.getActivityObjectsInTimeline().size());
		// ########Sanity check
		...
		// /// REMOVE candidate timelines which are above the distance THRESHOLD. (actually here we remove the entry for such candidate timelines from the
		// distance scores map
		...
		// System.out.println("\nDebug note192_255: getActivityNamesGuidingRecommwithTimestamps() " + getActivityNamesGuidingRecommwithTimestamps() +
		// " size of current timeline="
		// + currentTimeline.getActivityObjectsInTimeline().size());
		...
		// System.out.println("\nDebug note192_263: getActivityNamesGuidingRecommwithTimestamps() " + getActivityNamesGuidingRecommwithTimestamps() +
		// " size of current timeline="
		// + currentTimeline.getActivityObjectsInTimeline().size());
		...
		// //////////////////////////////
		// System.out.println("\nDebug note192_282: getActivityNamesGuidingRecommwithTimestamps() " + getActivityNamesGuidingRecommwithTimestamps() +
		// " size of current timeline="
		// + currentTimeline.getActivityObjectsInTimeline().size());
		editDistancesSortedMapFullCand =
				(LinkedHashMap<Integer, Pair<String, Double>>) UtilityBelt.sortByValueAscendingIntStrDoub(editDistancesMapUnsortedFullCand); // Now distanceScoresSorted
		...
		if (caseType.equals("CaseBasedV1"))
		{
			...
			this.similarityOfEndPointActivityObjectCand = getCaseSimilarityEndPointActivityObjectCand(candidateTimelines,
					activitiesGuidingRecomm, caseType, userAtRecomm, this.dateAtRecomm.toString(), this.timeAtRecomm.toString());// getDistanceScoresforCandidateTimelines(candidateTimelines,activitiesGuidingRecomm);
		}
		...
		if (caseType.equals("CaseBasedV1"))
		{
			// rankScore= (1d-(editDistanceValExceptEnd/maxEditDistanceValExceptEnd))* (1d- (endPointActivityEditDistanceVal/maxEditDistanceValOfEnd));
			// createRankedTopRecommendedActivityNamesCaseBasedV1(this.topNextActivityObjects, this.similarityOfEndPointActivityObjectCand);
			
			// rankScore= (1d-(editDistanceValExceptEnd/maxEditDistanceValExceptEnd))* simEndPointActivityObject;
			createRankedTopRecommendedActivityNamesCaseBasedV1_3(this.topNextActivityObjects, this.similarityOfEndPointActivityObjectCand);
		}
		
		else
		{
			// createRankedTopRecommendedActivityNames
			createRankedTopRecommendedActivityNamesSimpleV3_3(this.topNextActivityObjects);// , this.userAtRecomm, dateAtRecomm, timeAtRecomm);
		}
		
		// System.out.println("Next recommended 5 activity is: "+this.topNextRecommendedActivities);
		
		/*
		 * IMPORTANT: If the next activity after the current activity object in the current timeline is an invalid activity, then we can include the current activity in the list of
		 * recommended activities, otherwise the current activity has to be removed from the list of recommended activities
		 */
		...
		// $$LOGGING WritingToFile.writeDistanceScoresSortedMapMU(this.userAtRecomm, this.dateAtRecomm, this.timeAtRecomm,
		// $$LOGGING this.editDistancesSortedMapFullCand, this.candidateTimelines, this.topNextActivityObjects, this.activitiesGuidingRecomm,
		// $$LOGGING Constant.WriteCandInEditDistancePerRtPerCand, Constant.WriteEditOperatationsInEditDistancePerRtPerCand);
		
		// $$LOGGING System.out.println(
		// $$LOGGING "\nDebug note192_end: getActivityNamesGuidingRecommwithTimestamps() " + getActivityNamesGuidingRecommwithTimestamps()
		// $$LOGGING + " size of current timeline=" + currentTimeline.getActivityObjectsInTimeline().size());
		// System.out.println("Debug note192_2: current timeline " + currentTimeline.getActivityObjectNamesInSequence());
	...
	public RecommendationMasterMU(LinkedHashMap<Date, UserDayTimeline> trainingTimelines,
			LinkedHashMap<Date, UserDayTimeline> testTimelines, String dateAtRecomm, String timeAtRecomm, int userAtRecomm,
			double thresholdVal, String typeOfThreshold, double matchingUnitInCountsOrHours, String caseType, String lookPastType // counts or hours
	)
	{
		...
		WritingToFile.writeDistanceScoresSortedMapMU(this.userAtRecomm, this.dateAtRecomm, this.timeAtRecomm,
				this.editDistancesSortedMapFullCand, this.candidateTimelines, this.topNextActivityObjects, this.activitiesGuidingRecomm,
				Constant.WriteCandInEditDistancePerRtPerCand, Constant.WriteEditOperatationsInEditDistancePerRtPerCand);
	 ...
	 * 
	 * and calls the following setter methods setRecommendedActivityNamesWithRankscores setRankedRecommendedActivityNamesWithRankScores
	...
	public void createRankedTopRecommendedActivityNamesCaseBasedV1_2(
			...
			LinkedHashMap<Integer, Double> similarityOfEndPointActivityObjectCand) // we might remove these arguments as these are already member variables of
																					// this class
	{
		...
		for (int i = 0; i < numberOfTopNextActivityObjects; i++)
		{
			...
			double normEditDistanceValExceptEnd =
					minMaxNorm(editDistanceValExceptEnd, maxEditDistanceValExceptEnd, minEditDistanceValExceptEnd);
			...
			if (Constant.rankScoring.trim().equalsIgnoreCase("product"))
			{
				...
				System.out.println(
						"Prod RANK SCORE CALC=" + "(1-(" + normEditDistanceValExceptEnd + "))* (" + simEndPointActivityObject + ")");
			}
			
			else
			{
				simRankScore = Constant.ALPHA * (1d - normEditDistanceValExceptEnd) + (1 - Constant.ALPHA) * simEndPointActivityObject;
				...
				// simRankScore = Constant.BETA * (1d - (editDistanceValExceptEnd / maxEditDistanceValExceptEnd)) + (1 - Constant.BETA) *
				// simEndPointActivityObject;
				// System.out.println("Sum RANK SCORE CALC=" + Constant.BETA + "*(1-(" + editDistanceValExceptEnd + "/" + maxEditDistanceValExceptEnd + "))" +
				// "" + "+" + (1 - Constant.BETA) +
				// "*("
				// + simEndPointActivityObject + ")");
			}
	 ...
	 * Is function with Constants Beta and rank scoring
	 * 
	 * @since IN VERSION 2 WE HAD MIN MAX NORMALISATION INSIDE THIS FUNCTION, IN THIS V3 WE WILL NOT HAVE NORMALISATION OF EDIT DISTANCE INSIDE THIS FUNCTION AS THE NORMALISATION
	 *        IS DONE BEFOREHAND IN THE METHOD WHICH FETCHED THE NORMALISED EDIT DISTANCE FOR CANDIDATE TIMELINES
	 * 
	 * @param topNextActivityObjectsWithDistance
	 */
	...
	public double getRestAndEndSimilaritiesCorrelation()
	{
		...
		return (pc.correlation(UtilityBelt.toPrimitive(normEditSimilarity), UtilityBelt.toPrimitive(simEndActivityObjForCorr)));
	}
	...
	public void createRankedTopRecommendedActivityNamesSimpleV3_2(
			ArrayList<Triple<ActivityObject, Double, Integer>> topNextActivityObjectsWithDistance, String userAtRecomm, String dateAtRecomm,
			String timeAtRecomm)
	// LinkedHashMap<Integer, Double> similarityOfEndPointActivityObjectCand) // we might remove these arguments as these are already member variables of this
	// class
	{
		String topRankedActivityNamesWithScore, topRankedActivityNamesWithoutScore;
		...
		{
			String toWrite =
					userAtRecomm + "," + dateAtRecomm + "," + timeAtRecomm + "," + editDistances + "," + normalisedEditDistances + "\n";
	...
	/**
	 * Returns min max norm if max - min >0 else return 0 (as distance) ...leading to 1 as similarity (rounded off to 4 decimal places)
	 * 
	 * @param val
	 * @param max
	 * @param min
	 ...
	 * 
	 * public String getTopRecommendedActivitiesWithoutDistance() { String result=""; String topActivities=this.topNextRecommendedActivities; String[] splitted1=
	 * topActivities.split("__");
	 * 
	 * for(int i=1;i<splitted1.length;i++) { String[] splitted2=splitted1[i].split(":"); result=result+"__"+splitted2[0]; }
	 * 
	 * return result; }
	 */
	// //
	...
	{
		ArrayList<Triple<ActivityObject, Double, Integer>> topActivityObjects = new ArrayList<Triple<ActivityObject, Double, Integer>>(); // Pair <Next Activity
																																			// Object,edit
																																			// distance,
																																			// TimelineID>
		...
		for (Map.Entry<Integer, Pair<String, Double>> entryInSim : editDistanceSortedFullCand.entrySet())
		{
			...
			topActivityObjects.add(new Triple<ActivityObject, Double, Integer>(simCandidateTimeline.getNextActivityObject(),
					editDistanceForSimCandidate, simCandidateID)); // take the next
																	// activity
																	// object (next
																	// activity
																	// object is the valid
	...
	// ////////
	/*
	 * Added: Oct 5, 2014: for IMPORTANT POINT: THE CANDIDATE TIMELINE IS THE DIRECT CANDIDATE TIMELINE AND NOT THE LEAST DISTANT SUBCANDIDATE.
	 */
	/**
	 * Returns a map where each entry corresponds to a candidate timeline. The value of an entry is the edit distance of that candidate timeline with the current timeline.
	 * 
	 * @param candidateTimelines
	...
	public LinkedHashMap<Integer, Pair<String, Double>> getHJEditDistancesForCandidateTimelinesFullCand(
			LinkedHashMap<Integer, TimelineWithNext> candidateTimelines, ArrayList<ActivityObject> activitiesGuidingRecomm, String caseType,
		...
		for (Map.Entry<Integer, TimelineWithNext> entry : candidateTimelines.entrySet())
		{
			...
			switch (caseType)
			{
				case "CaseBasedV1":
					...
					if (Constant.EXPUNGE_INVALIDS_B4_RECOMM_PROCESS) // invalids are already expunged, no need to expunge again
					{
						editDistanceForThisCandidate =
								hjEditDistance.getHJEditDistanceWithoutEndCurrentActivity(entry.getValue().getActivityObjectsInTimeline(),
										activitiesGuidingRecomm, userAtRecomm, dateAtRecomm, timeAtRecomm, new Long(candidateTimelineId));
					}
					else
					{
						editDistanceForThisCandidate = hjEditDistance.getHJEditDistanceWithoutEndCurrentActivityInvalidsExpunged(
								entry.getValue().getActivityObjectsInTimeline(), activitiesGuidingRecomm, userAtRecomm, dateAtRecomm,
								timeAtRecomm, new Long(candidateTimelineId));
					}
					break;
				...
				case "SimpleV3":
					...
					{
						editDistanceForThisCandidate =
								hjEditDistance.getHJEditDistanceWithTrace(entry.getValue().getActivityObjectsInTimeline(),
										activitiesGuidingRecomm, userAtRecomm, dateAtRecomm, timeAtRecomm, new Long(candidateTimelineId));
					}
					else
					{
						editDistanceForThisCandidate =
								hjEditDistance.getHJEditDistanceInvalidsExpunged(entry.getValue().getActivityObjectsInTimeline(),
										activitiesGuidingRecomm, userAtRecomm, dateAtRecomm, timeAtRecomm, new Long(candidateTimelineId));
					}
					break;
			 ...
			 * editSimilarity.getEditDistanceWithoutEndCurrentActivity(entry.getValue().getActivityObjectsInTimeline(),activitiesGuidingRecomm); } else //SimpleV3 {
			 * editDistanceForThisCandidate = editSimilarity.getEditDistance(entry.getValue().getActivityObjectsInTimeline(),activitiesGuidingRecomm); }
			 */
	...
	{
		// <CandidateTimeline ID, Edit distance>
		LinkedHashMap<Integer, LinkedHashMap<String, Pair<String, Double>>> candEditDistancesFeatureWise =
				new LinkedHashMap<Integer, LinkedHashMap<String, Pair<String, Double>>>();
		...
		for (Map.Entry<Integer, TimelineWithNext> entry : candidateTimelines.entrySet())
		{
			...
			switch (caseType)
			{
				case "CaseBasedV1":
					...
					if (Constant.EXPUNGE_INVALIDS_B4_RECOMM_PROCESS) // invalids are already expunged, no need to expunge again
					{
						featureWiseEditDistancesForThisCandidate =
								featureWiseEditDistance.getFeatureWiseEditDistanceWithoutEndCurrentActivity(
										entry.getValue().getActivityObjectsInTimeline(), activitiesGuidingRecomm);// , userAtRecomm, dateAtRecomm, timeAtRecomm,
																													// candidateTimelineId);
					}
					else
					{
						featureWiseEditDistancesForThisCandidate =
								featureWiseEditDistance.getFeatureWiseEditDistanceWithoutEndCurrentActivityInvalidsExpunged(
										entry.getValue().getActivityObjectsInTimeline(), activitiesGuidingRecomm);// , userAtRecomm, dateAtRecomm, timeAtRecomm,
																													// candidateTimelineId);
					}
					break;
				...
				case "SimpleV3":
					...
					if (Constant.EXPUNGE_INVALIDS_B4_RECOMM_PROCESS)
					{
						featureWiseEditDistancesForThisCandidate = featureWiseEditDistance.getFeatureWiseEditDistanceWithTrace(
								entry.getValue().getActivityObjectsInTimeline(), activitiesGuidingRecomm);// , userAtRecomm, dateAtRecomm, timeAtRecomm, candidateTimelineId);
					}
					else
					{
						featureWiseEditDistancesForThisCandidate = featureWiseEditDistance.getFeatureWiseEditDistanceInvalidsExpunged(
								entry.getValue().getActivityObjectsInTimeline(), activitiesGuidingRecomm);// , userAtRecomm, dateAtRecomm, timeAtRecomm, candidateTimelineId);
					}
					break;
	...
	/**
	 * Returns a map where each entry corresponds to a candidate timeline. The value of an entry is the OTMDSAM edit distance of that candidate timeline with the current timeline.
	 * 
	 * @param candidateTimelines
	 * @param activitiesGuidingRecomm
	 * @param caseType
		...
		for (Map.Entry<Integer, TimelineWithNext> entry : candidateTimelines.entrySet())
		{
			...
			switch (caseType)
			{
				case "CaseBasedV1":
					...
					{
						editDistanceForThisCandidate = OTMDSAMEditDistance.getOTMDSAMEditDistanceWithoutEndCurrentActivityInvalidsExpunged(
								entry.getValue().getActivityObjectsInTimeline(), activitiesGuidingRecomm, userAtRecomm, dateAtRecomm,
								timeAtRecomm, candidateTimelineId);
					}
					break;
				...
				case "SimpleV3":
					...
					{
						editDistanceForThisCandidate =
								OTMDSAMEditDistance.getOTMDSAMEditDistanceWithTrace(entry.getValue().getActivityObjectsInTimeline(),
										activitiesGuidingRecomm, userAtRecomm, dateAtRecomm, timeAtRecomm, candidateTimelineId);
					}
					else
					{
						editDistanceForThisCandidate =
								OTMDSAMEditDistance.getOTMDSAMEditDistanceInvalidsExpunged(entry.getValue().getActivityObjectsInTimeline(),
										activitiesGuidingRecomm, userAtRecomm, dateAtRecomm, timeAtRecomm, candidateTimelineId);
					}
					break;
	 ...
	 * @return
	 */
	public LinkedHashMap<Integer, LinkedHashMap<String, Pair<String, Double>>>
			getFeatureWiseWeightedEditDistancesForCandidateTimelinesFullCand(LinkedHashMap<Integer, TimelineWithNext> candidateTimelines,
					ArrayList<ActivityObject> activitiesGuidingRecomm, String caseType, String userAtRecomm, String dateAtRecomm,
					String timeAtRecomm)
	{
		// <CandidateTimeline ID, Edit distance>
		LinkedHashMap<Integer, LinkedHashMap<String, Pair<String, Double>>> candEditDistancesFeatureWise =
		...
		for (Map.Entry<Integer, TimelineWithNext> entry : candidateTimelines.entrySet())
		{
			...
			switch (caseType)
			{
				case "CaseBasedV1":
					...
					if (Constant.EXPUNGE_INVALIDS_B4_RECOMM_PROCESS) // invalids are already expunged, no need to expunge again
					{
						featureWiseWeightedEditDistancesForThisCandidate =
								featureWiseWeightedEditDistance.getFeatureWiseWeightedEditDistanceWithoutEndCurrentActivity(
										entry.getValue().getActivityObjectsInTimeline(), activitiesGuidingRecomm);// , userAtRecomm, dateAtRecomm, timeAtRecomm,
																													// candidateTimelineId);
					}
					else
					{
						featureWiseWeightedEditDistancesForThisCandidate =
								featureWiseWeightedEditDistance.getFeatureWiseWeightedEditDistanceWithoutEndCurrentActivityInvalidsExpunged(
										entry.getValue().getActivityObjectsInTimeline(), activitiesGuidingRecomm);// , userAtRecomm, dateAtRecomm, timeAtRecomm,
																													// candidateTimelineId);
					}
					break;
				...
				case "SimpleV3":
					...
					{
						featureWiseWeightedEditDistancesForThisCandidate =
								featureWiseWeightedEditDistance.getFeatureWiseWeightedEditDistanceRawValsWithTrace(
										entry.getValue().getActivityObjectsInTimeline(), activitiesGuidingRecomm);// , userAtRecomm, dateAtRecomm, timeAtRecomm,
																													// candidateTimelineId);
					}
					else
					{
						featureWiseWeightedEditDistancesForThisCandidate =
								featureWiseWeightedEditDistance.getFeatureWiseWeightedEditDistanceInvalidsExpunged(
										entry.getValue().getActivityObjectsInTimeline(), activitiesGuidingRecomm);// , userAtRecomm, dateAtRecomm, timeAtRecomm,
																													// candidateTimelineId);
					}
					break;
	...
	/**
	 * Checks for the distance used and calls the appropriate method for getting normalised distance for candidate timelines.
	 * 
	 * @param candidateTimelines
	 * @param activitiesGuidingRecomm
	 * @param caseType
	...
	public LinkedHashMap<Integer, Pair<String, Double>> getNormalisedDistancesForCandidateTimelinesFullCand(
			LinkedHashMap<Integer, TimelineWithNext> candidateTimelines, ArrayList<ActivityObject> activitiesGuidingRecomm, String caseType,
			String userAtRecomm, String dateAtRecomm, String timeAtRecomm, String distanceUsed)
	{
		
		switch (distanceUsed)
		{
			case "HJEditDistance":
				return getNormalisedHJEditDistancesForCandidateTimelinesFullCand(candidateTimelines, activitiesGuidingRecomm, caseType,
						this.userIDAtRecomm, this.dateAtRecomm.toString(), this.timeAtRecomm.toString());
			case "FeatureWiseEditDistance":
				return getNormalisedFeatureWiseEditDistancesForCandidateTimelinesFullCand(candidateTimelines, activitiesGuidingRecomm,
						caseType, this.userIDAtRecomm, this.dateAtRecomm.toString(), this.timeAtRecomm.toString());
			
			case "FeatureWiseWeightedEditDistance":
				return getNormalisedFeatureWiseWeightedEditDistancesForCandidateTimelinesFullCand(candidateTimelines,
						activitiesGuidingRecomm, caseType, this.userIDAtRecomm, this.dateAtRecomm.toString(), this.timeAtRecomm.toString());
			
			case "OTMDSAMEditDistance":
				return getNormalisedOTMDSAMEditDistancesForCandidateTimelinesFullCand(candidateTimelines, activitiesGuidingRecomm, caseType,
						this.userIDAtRecomm, this.dateAtRecomm.toString(), this.timeAtRecomm.toString());
			// break;
			default:
				PopUps.showError(
		...
		System.err.println("Error in org.activity.recomm.RecommendationMasterMU.getNormalisedDistancesForCandidateTimelinesFullCand()"
	 ...
	 * Returns a map where each entry corresponds to a candidate timeline. The value of an entry is the edit distance of that candidate timeline with the current timeline.
	 * 
	 * DOING THE NORMALISATION HERE ITSELF AND SEE IF IT GIVES DIFFERENT RESULT THAN DOING NORMALISATION WHILE CALCULATING SCORE. CHECKED: SAME RESULTS, NORMALISATION CORRECT
	 * 
	 * @param candidateTimelines
	 * @param activitiesGuidingRecomm
	 * @param caseType
	...
	public LinkedHashMap<Integer, Pair<String, Double>> getNormalisedHJEditDistancesForCandidateTimelinesFullCand(
			...
			String userAtRecomm, String dateAtRecomm, String timeAtRecomm)
	{
		LinkedHashMap<Integer, Pair<String, Double>> candEditDistances = getHJEditDistancesForCandidateTimelinesFullCand(candidateTimelines,
				activitiesGuidingRecomm, caseType, userAtRecomm, dateAtRecomm, timeAtRecomm);
		
		LinkedHashMap<Integer, Pair<String, Double>> normalisedCandEditDistances =
				normalisedDistancesOverTheSet(candEditDistances, userAtRecomm, dateAtRecomm, timeAtRecomm);
	...
	{
		LinkedHashMap<Integer, LinkedHashMap<String, Pair<String, Double>>> candEditDistancesFeatureWise =
				getFeatureWiseEditDistancesForCandidateTimelinesFullCand(candidateTimelines, activitiesGuidingRecomm, caseType,
		...
		LinkedHashMap<Integer, LinkedHashMap<String, Pair<String, Double>>> normalisedCandEditDistances =
				normalisedFeatureWiseDistancesOverTheSet(candEditDistancesFeatureWise);
		LinkedHashMap<Integer, Pair<String, Double>> aggregatedNormalisedCandEditDistances =
	...
	{
		LinkedHashMap<Integer, LinkedHashMap<String, Pair<String, Double>>> candEditDistancesFeatureWise =
				getFeatureWiseWeightedEditDistancesForCandidateTimelinesFullCand(candidateTimelines, activitiesGuidingRecomm, caseType,
						userAtRecomm, dateAtRecomm, timeAtRecomm);
	...
	public void traverseLLP(LinkedHashMap<Integer, LinkedHashMap<String, Pair<String, Double>>> map, String name)
	{
		...
		for (Map.Entry<Integer, LinkedHashMap<String, Pair<String, Double>>> entry : map.entrySet()) // iterating over cands
		{
			System.out.print("Cand=" + entry.getKey());
			LinkedHashMap<String, Pair<String, Double>> featureWiseDistances = entry.getValue();
			
			for (Map.Entry<String, Pair<String, Double>> distEntry : featureWiseDistances.entrySet()) // iterating over distance for each feature
			{
	...
	public void traverseLP(LinkedHashMap<Integer, Pair<String, Double>> map, String name)
	{
		...
		for (Map.Entry<Integer, Pair<String, Double>> distEntry : map.entrySet()) // iterating over distance for each feature
		{
			...
			LinkedHashMap<Integer, LinkedHashMap<String, Pair<String, Double>>> setOfFeatureWiseDistances)
	{
		...
		for (Map.Entry<Integer, LinkedHashMap<String, Pair<String, Double>>> entry : setOfFeatureWiseDistances.entrySet()) // iterating over cands
		{
			...
			LinkedHashMap<Integer, LinkedHashMap<String, Pair<String, Double>>> normalisedCandEditDistances)
	{
		...
		for (Map.Entry<Integer, LinkedHashMap<String, Pair<String, Double>>> entry : normalisedCandEditDistances.entrySet()) // iterating over cands
		{
			Integer candID = entry.getKey();
			...
			for (Map.Entry<String, Pair<String, Double>> distEntry : normalisedFeatureWiseDistances.entrySet()) // iterating over distance for each feature
			{
			...
			distanceAggregatedOverFeatures = UtilityBelt.round(distanceAggregatedOverFeatures / Constant.getNumberOfFeatures(), 4);
	...
	public LinkedHashMap<Integer, Pair<String, Double>> normalisedDistancesOverTheSet(
			LinkedHashMap<Integer, Pair<String, Double>> setOfDistances, String userAtRecomm, String dateAtRecomm, String timeAtRecomm)
	{
		...
		System.out.println("getNormalisedDistancesOverTheSet: #Vals max=" + numOfValsAtMax + " #Vals min=" + numOfValsAtMin);
		return normalisedDistances;
	}
	
	// ////////
	/*
	 * Added: Oct 7, 2014: for IMPORTANT POINT: THE CANDIDATE TIMELINE IS THE DIRECT CANDIDATE TIMELINE AND NOT THE LEAST DISTANT SUBCANDIDATE.
	 */
	/**
	 * Returns a map where each entry corresponds to a candidate timeline. The value of an entry is the edit distance value between the end point activity object (current activity)
	 * of the candidate timeline and the current activity object (activity at recomm point)
	 * 
	 * @param candidateTimelines
	 * @param activitiesGuidingRecomm
	 * @param caseType
	 ...
	 * @param timeAtRecomm
	 *            only for the purpose of writing to file
	 * @return <CanditateTimelineID to which this end point Activity Object belongs, edit distance of this end point Activity Object of this candidate with end point Activity
	 *         Object>
	 */
	public LinkedHashMap<Integer, Double> getCaseSimilarityEndPointActivityObjectCand(
			LinkedHashMap<Integer, TimelineWithNext> candidateTimelines, ArrayList<ActivityObject> activitiesGuidingRecomm, String caseType,
			int userID, String dateAtRecomm, String timeAtRecomm)
	{
		// <CandidateTimeline ID, Edit distance>
		...
		for (Map.Entry<Integer, TimelineWithNext> entry : candidateTimelines.entrySet())
		{
			...
			{
				ActivityObject endPointActivityObjectCandidate = (activityObjectsInCand.get(activityObjectsInCand.size() - 1)); // only the end point activity
																																// object
				ActivityObject endPointActivityObjectCurrentTimeline = (activitiesGuidingRecomm.get(activitiesGuidingRecomm.size() - 1)); // activityObjectAtRecommPoint
				...
				// (Constant.DATABASE_NAME)
				{
					...
					case "dcu_data_2":
						...
						break;
					default:
						System.err.println("Error in getCaseSimilarityEndPointActivityObjectCand: unrecognised database name");
						break;
				}
			...
			else if (caseType.equals("SimpleV3"))
			{
				// editDistanceForThisCandidate = editSimilarity.getEditDistance(entry.getValue().getActivityObjectsInTimeline(),activitiesGuidingRecomm);
				System.err
						.println("ERROR in getEditDistancesOfEndPointActivityCand(): This method should not have been called for case type="
								+ caseType);
				errorExists = true;
			...
			{
				System.err.println("Error in getCaseSimilarityEndPointActivityObjectCand: endPointEditDistanceForThisCandidate "
	...
	public String getStringCodeOfActivityObjects(ArrayList<ActivityObject> activityObjects)
	{
		...
		for (int i = 0; i < activityObjects.size(); i++)
		{
			...
			code += activityObjects.get(i).getStringCode(); // Character.toString ((char)(activityID+65)); //getting the ascii code for (activity id+65)
		}
	 ...
	 * @return
	 */
	public LinkedHashMap<Integer, TimelineWithNext> getCandidateTimelinesMU(Timeline trainingTimeline, double matchingUnit,
			String lookPastType)
	{
		if (lookPastType.equalsIgnoreCase("Count"))
		{
	...
	/**
	 * Create and fetch candidate timelines from the training timelines. Finding Candidate timelines: iterate through the training timelines for each occurence of the Current
	 * Activity Name in the candidate timeline, extract the sequence of activity objects from that occurrence_index back until the matching unit number of activity objects this
	 * forms a candidate timeline
	 * 
	 * @param dayTimelinesForUser
	 * @return
	 */
	public LinkedHashMap<Integer, TimelineWithNext> getCandidateTimelinesMUCount(Timeline trainingTimeline, int matchingUnitInCounts)// ,
																																		...
																																		// //Date//dateAtRecomm)
	{
		...
		// $$System.out.println("Current activity (activityAtRecommPoint)=" + this.activityAtRecommPoint.getActivityName());
		// trainingTimeline.printActivityObjectNamesWithTimestampsInSequence();
		
		/**
		 * Note: candidate timelines can be formed from the first index of the training timeline UNLIKE matching unit in hours
		 */
		for (int i = 0; i < activityObjectsInTraining.size() - 1; i++) // starting from the first activity and goes until second last activity.
		{
			...
			{
				// Timestamp newCandEndTimestamp= new Timestamp(ae.getStartTimestamp().getTime()+ae.getDurationInSeconds()*1000-1000); //decreasing 1 second
				// (because this is convention followed
				// in
				// data generation)
				int newCandEndIndex = i;
				// NOTE: going back matchingUnitCounts FROM THE index.
				int newCandStartIndex = (newCandEndIndex - matchingUnitInCounts) >= 0 ? (newCandEndIndex - matchingUnitInCounts) : 0;
				
				// $$System.out.println("\n\tStart index of candidate timeline=" + newCandStartIndex);
				// $$System.out.println("\tEnd index of candidate timeline=" + newCandEndIndex);
				
				ArrayList<ActivityObject> activityObjectsForCandidate =
						trainingTimeline.getActivityObjectsInTimelineFromToIndex(newCandStartIndex, newCandEndIndex + 1);
				// getActivityObjectsBetweenTime(newCandStartTimestamp,newCandEndTimestamp);
				ActivityObject nextValidActivityForCandidate =
						trainingTimeline.getNextValidActivityAfterActivityAtThisPosition(newCandEndIndex);
				...
				TimelineWithNext newCandidate = new TimelineWithNext(activityObjectsForCandidate, nextValidActivityForCandidate);// trainingTimeline.getActivityObjectsBetweenTime(newCandStartTimestamp,newCandEndTimestamp));
	 ...
	 * @return
	 */
	public LinkedHashMap<Integer, TimelineWithNext> getCandidateTimelinesMUHours(Timeline trainingTimeline, double matchingUnitInHours)// ,
																																		// /*ArrayList<ActivityObject>
																																		// activitiesGuidingRecomm,*/
																																		// //Date//dateAtRecomm)
	{
		...
		for (int i = 1; i < activityObjectsInTraining.size() - 1; i++) // starting from the second activity and goes until second last activity.
		{
			ActivityObject ae = activityObjectsInTraining.get(i);
			
			if (ae.getActivityName().equals(this.activityNameAtRecommPoint)) // same name as current activity
			{
				Timestamp newCandEndTimestamp = new Timestamp(ae.getStartTimestamp().getTime() + ae.getDurationInSeconds() * 1000 - 1000); // decreasing 1
																																			// second (because
																																			// this is
																																			// convention
																																			// followed
				...
				long matchingUnitInMilliSeconds = (long) (matchingUnitInHours * 60 * 60 * 1000);// .multiply(new BigDecimal(60*60*1000)).longValue();
				...
				/*
				 * Note: if newCandStartTimestamp here is earlier than when the training timeline started, even then this works correctly since we are considering intersection of
				 * what is available
				 */
				ArrayList<ActivityObject> activityObjectsForCandidate =
						trainingTimeline.getActivityObjectsBetweenTime(newCandStartTimestamp, newCandEndTimestamp);
				ActivityObject nextValidActivityForCandidate =
						trainingTimeline.getNextValidActivityAfterActivityAtThisTime(newCandEndTimestamp);
	...
	public ArrayList<Integer> getIndicesOfEndPointActivityInTimeline(String userActivitiesAsStringCode, String codeOfEndPointActivity)
	{
		// System.out.println("\nDebug getIndicesOfEndPointActivityInDayButNotAsLast: userDayActivitiesAsStringCode="+userDayActivitiesAsStringCode+" and
		// codeOfEndPointActivity="+codeOfEndPointActivity);
	...
	public void traverseMapOfTimelinesWithNext(LinkedHashMap<Integer, TimelineWithNext> map)
	{
		...
		for (Map.Entry<Integer, TimelineWithNext> entry : map.entrySet())
		{
			System.out.print("ID: " + entry.getKey());
			entry.getValue().printActivityObjectNamesInSequence();
			System.out.println("\t ** Next Activity Object (name=): " + entry.getValue().getNextActivityObject().getActivityName());
		}
		System.out.println("-----------");
	}
	...
	public long getSumOfActivityObjects(LinkedHashMap<Integer, TimelineWithNext> map)
	{
		...
		for (Map.Entry<Integer, TimelineWithNext> entry : map.entrySet())
		{
			...
			if (a != b)
			{
				PopUps.showError("Error in getSumOfActivityObjects a should be equal to be since we removed invalid aos beforehand but a = "
						+ a + " and b=" + b);
			}
			count += a;
		}
	 ...
	 * @return
	 */
	public int traverseCandidateTimelineWithEditDistance()// LinkedHashMap<Integer, Pair<String, Double>> editDistancesSortedMapFullCand)
	{
		...
		{
			System.out.println("Warning in traverseCandidateTimelineWithEditDistance(): editDistancesSortedMapFullCand is empty.");
		}
		return 0;
	}
	...
	public int getNumberOfValidActivitiesInActivitesGuidingRecommendation()
	{
		...
		for (ActivityObject ae : this.activitiesGuidingRecomm)
		{
			...
			// (!( (ae.getActivityName().equalsIgnoreCase("Others")) || (ae.getActivityName().equalsIgnoreCase("Unknown")) ))
			{
				count++;
			}
		}
 ...
 * 
 * setRecommendedActivityNamesRankscorePairs setRankedRecommendedActivityNamesWithRankScores setRankedRecommendedActivityNamesWithoutRankScores
 * 
 * @param topNextActivityObjectsWithDistance
 */
// public void createRankedTopRecommendedActivityNamesCaseBasedV1(ArrayList<Triple<ActivityObject, Double, Integer>> topNextActivityObjectsWithDistance,
// LinkedHashMap<Integer, Double> editDistanceOfEndPointActivityObjectCand) // we might remove these arguments as these are already member variables of this
// class
// {
// String topRankedActivityNamesWithScore, topRankedActivityNamesWithoutScore;
//
...
//
// // System.out.println("Debug inside createRankedTopRecommendedActivityObjects: topRecommendationsWithDistance="+topRecommendationsWithDistance);
// System.out.println("Debug inside createRankedTopRecommendedActivityObjects: numberOfTopNextActivityEvenst=numberOfTopNextActivityEvenst");
// // System.out.print("Debug inside createRankedTopRecommendedActivityObjects: the read next activity objects are: ");
//
// LinkedHashMap<String, Double> recommendedActivityNamesRankscorePairs = new LinkedHashMap<String, Double>(); // <ActivityName,RankScore>
//
...
//
// Double rankScore = (1d - (editDistanceValExceptEnd / maxEditDistanceValExceptEnd)) * (1d - (endPointActivityEditDistanceVal / maxEditDistanceValOfEnd));
//
// System.out.println("RANK SCORE CALCULATION=" + "(1d-(" + editDistanceValExceptEnd + "/" + maxEditDistanceValExceptEnd + "))* (1d- (" +
...
// else
// {
// recommendedActivityNamesRankscorePairs.put(topNextActivityName, recommendedActivityNamesRankscorePairs.get(topNextActivityName) + rankScore);
// }
// }
//
// System.out.println();
//
// recommendedActivityNamesRankscorePairs = (LinkedHashMap<String, Double>) UtilityBelt.sortByValue(recommendedActivityNamesRankscorePairs); // Sorted in
...
// topRankedActivityNamesWithScore = getRankedRecommendedActivityNamesWithRankScores();
// topRankedActivityNamesWithoutScore = getRankedRecommendedActivityNamesWithoutRankScores();
// System.out.println("Debug inside createRankedTopRecommendedActivityObjects: topRankedActivityNamesWithScore= " + topRankedActivityNamesWithScore);
// System.out.println("Debug inside createRankedTopRecommendedActivityObjects: topRankedActivityNamesWithoutScore= " + topRankedActivityNamesWithoutScore);
//
// // return topRankedString;
// }
 ...
 * @param topNextActivityObjectsWithDistance
 */
// public void createRankedTopRecommendedActivityNames(ArrayList<Triple<ActivityObject, Double, Integer>> topNextActivityObjectsWithDistance)
// {
// String topRankedActivityNamesWithScore, topRankedActivityNamesWithoutScore;
//
// int numberOfTopNextActivityObjects = topNextActivityObjectsWithDistance.size();
...
// */
// /**
// * Returns a map where each entry corresponds to a candidate timeline. The value of an entry is the edit distance value between the end point activity object
...
// * @param caseType
// * can be 'SimpleV3' or 'CaseBasedV1'
// * @return <CanditateTimelineID, edit distance of this end point Activity Object of this candidate with end point Activity Object>
// */
// public LinkedHashMap<Integer, Double> getEditDistancesOfEndPointActivityObjectCand(LinkedHashMap<Integer, TimelineWithNext> candidateTimelines,
// ArrayList<ActivityObject>
// activitiesGuidingRecomm,
// String caseType)
// {
...
// {
// // editDistanceForThisCandidate = editSimilarity.getEditDistance(entry.getValue().getActivityObjectsInTimeline(),activitiesGuidingRecomm);
// System.err.println("ERROR in getEditDistancesOfEndPointActivityCand(): This method should not have been called for case type=" + caseType);
// errorExists = true;
// endPointEditDistanceForThisCandidate = null;
// }
// else
...
// endPointActivityObjectCandidate.add(activityObjectsInCand.get(activityObjectsInCand.size() - 1)); // only the end point activity object
//
// ArrayList<ActivityObject> endPointActivityObjectCurrentTimeline = new ArrayList<ActivityObject>();
// endPointActivityObjectCurrentTimeline.add(activitiesGuidingRecomm.get(activitiesGuidingRecomm.size() - 1)); // activityObjectAtRecommPoint
//
// endPointEditDistanceForThisCandidate = editSimilarity.getEditDistance(endPointActivityObjectCandidate, endPointActivityObjectCurrentTimeline);
// }
//
// candEndPointEditDistances.put(entry.getKey(), endPointEditDistanceForThisCandidate);
// // System.out.println("now we put "+entry.getKey()+" and score="+score);
