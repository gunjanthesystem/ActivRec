 ...
 * @author Alex Black
 * 
 *         Example: Train a LSTM RNN to generates text, one character at a time. This example is somewhat inspired by
 *         Andrej Karpathy's blog post, "The Unreasonable Effectiveness of Recurrent Neural Networks"
 *         http://karpathy.github.io/2015/05/21/rnn-effectiveness/
 * 
 *         This example is set up to train on the Complete Works of William Shakespeare, downloaded from Project
 *         Gutenberg. Training on other text sources should be relatively easy to implement.
 * 
 ...
 */
public class LSTMCharModellingExample_WC1
{
	public static void main(String[] args) throws Exception
	{
		...
		// our LSTM network.
		CharacterIterator_WC1 iter = getShakespeareIterator(miniBatchSize, exampleLength);
		...
		MultiLayerConfiguration conf = new NeuralNetConfiguration.Builder().seed(12345).l2(0.001)
				.weightInit(WeightInit.XAVIER).updater(new RmsProp(0.1)).list()
				.layer(0,
		...
		for (int epochCount = 0; epochCount < numEpochs; epochCount++)
		{
			...
			while (iter.hasNext())
			{
				...
				// Generate sample outputs (sample texts) at the current level of training (i.e., after completion of n
				// mini-bactches of 32x1000 charcters)
	 ...
	 */
	public static CharacterIterator_WC1 getShakespeareIterator(int miniBatchSize, int sequenceLength) throws Exception
	{
		System.out.println("Inside getShakespeareIterator(): miniBatchSize=" + miniBatchSize + " sequenceLength="
				+ sequenceLength);
		...
		char[] validCharacters = CharacterIterator_WC1.getMinimalCharacterSet(); // Which characters are allowed? Others
																					// will be removed
		System.out.println("validCharacters= \n" + Arrays.toString(validCharacters) + "\n");

		CharacterIterator_WC1 res = new CharacterIterator_WC1(fileLocation, Charset.forName("UTF-8"), miniBatchSize,
				sequenceLength, validCharacters, new Random(12345));

		System.out.println("Exiting getShakespeareIterator()");
		return res;
	}

	/**
	 * Generate a sample from the network, given an (optional, possibly null) initialization. Initialization can be used
	 * to 'prime' the RNN with a sequence you want to extend/continue.<br>
	 ...
	 *            Number of characters to sample from network (excluding initialization)
	 * @param net
	 *            MultiLayerNetwork with one or more LSTM/RNN layers and a softmax output layer
	 * @param iter
	 *            CharacterIterator. Used for going from indexes back to characters
	 */
	private static String[] sampleCharactersFromNetwork(String initialization, MultiLayerNetwork net,
			CharacterIterator_WC1 iter, Random rng, int charactersToSample, int numSamples)
	{
	...
	/**
	 * Given a probability distribution over discrete classes, sample from the distribution and return the generated
	 * class index.
	 * 
	 * @param distribution
