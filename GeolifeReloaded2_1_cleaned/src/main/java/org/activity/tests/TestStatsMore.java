...
public class TestStatsMore
{
	...
	public static void writePrecisionRecallFMeasure(String fileNamePhrase, String timeCategory, int theKOriginal, ArrayList<ArrayList<String>> arrayMeta, ArrayList<ArrayList<String>> arrayTopK, ArrayList<ArrayList<String>> arrayActual)
	{
		...
		try
		{
			...
			/*
			 * brMeta = new BufferedReader(new FileReader("/home/gunjan/meta.csv")); brTopK = new BufferedReader(new FileReader("/home/gunjan/dataRecommTop5.csv")); brActual = new BufferedReader(new FileReader("/home/gunjan/dataActual.csv"));
			 */
			...
			System.out.println("size of meta array=" + arrayMeta.size() + "     size of topK array=" + arrayTopK.size() + "   size of actual array=" + arrayActual.size());
			...
			for (int i = 0; i < arrayMeta.size(); i++) // iterating over users
			{
				...
				for (int j = 0; j < currentLineArray.size(); j++) // iterating over recommendation times
				{
					...
					if (getTimeCategoryOfTheDay(hourOfTheDay).equalsIgnoreCase(timeCategory) || timeCategory.equals("All"))
					{
						...
						String[] topKStrings = arrayTopK.get(i).get(j).split("__"); // __a__b__c__d__e is of length 6... value at index 0 is empty.
						...
						{
							System.err.println("Warning: For " + currentLineArray.get(j) + ", Only top " + (topKStrings.length - 1) + " recommendation present while the asked for K is " + theK
						...
						else
						{
							...
							System.out.println("count-of-occurence-used=" + countOfOccurence + "         " + "k-used=" + theK + " k-Original=" + theKOriginal);// + " / ="+round((double)countOfOccurence/theK,4));
						}
						...
						System.out.println("top-" + theKOriginal + "-precision=" + topKPrecisionVal + "    " + "top-" + theKOriginal + "-recall=" + topKRecallVal + "   top" + theKOriginal + "F=" + topKFVal + "   accuracy=" + accuracy);
					}
			...
			for (int user = 0; user < numUsers; user++)
			{
				...
				for (int K = theKOriginal; K > 0; K--)
				{
					...
					while ((currentLine = br.readLine()) != null)
					{
						if (lineNumber == user)
						{
							String[] pValuesForThisUserForThisK = currentLine.split(","); // get avg of all these precision values concern: splitting on empty lines gives an array of length1, also splitting on one values line gives an array of length 1
							...
							{
								System.out.println(" NOTE: line for user(" + user + ") is EMPTY for " + commonPath + fileNamePhrase + timeCategory + "top" + K + "Precision.csv");
							}
	 ...
	 * String currentLine;
	 * 
	 * while ((currentLine = br.readLine()) != null) { String[] rValuesForThisUserForThisK=currentLine.split(","); //double[] pValuesForThisUserForThisK = new double[tokensInCurrentLine.length];
	 * 
	 * double avgRValueForThisUserForThisK = 0; double sum=0;
	 * 
	 * for(int i=0;i<rValuesForThisUserForThisK.length;i++) { //pValuesForThisUserForThisK[i]=Double.parseDouble(tokensInCurrentLine[i]); sum = sum + Double.parseDouble(rValuesForThisUserForThisK[i]); }
	 * 
	 * avgRValueForThisUserForThisK = sum/rValuesForThisUserForThisK.length; System.out.println("Calculating avg recall (K="+K+"):"+sum+"/"+rValuesForThisUserForThisK.length); } br.close(); } bw.close(); }
	 * 
	 ...
	 * String currentLine;
	 * 
	 * while ((currentLine = br.readLine()) != null) { String[] fValuesForThisUserForThisK=currentLine.split(","); //double[] pValuesForThisUserForThisK = new double[tokensInCurrentLine.length];
	 * 
	 * double avgFValueForThisUserForThisK = 0; double sum=0;
	 * 
	 * for(int i=0;i<fValuesForThisUserForThisK.length;i++) { //pValuesForThisUserForThisK[i]=Double.parseDouble(tokensInCurrentLine[i]); sum = sum + Double.parseDouble(fValuesForThisUserForThisK[i]); }
	 * 
	 * avgFValueForThisUserForThisK = sum/fValuesForThisUserForThisK.length; System.out.println("Calculating avg recall (K="+K+"):"+sum+"/"+fValuesForThisUserForThisK.length); } br.close(); } bw.close(); }
	 * 
	 * catch (IOException e) { e.printStackTrace(); } }
	 */
