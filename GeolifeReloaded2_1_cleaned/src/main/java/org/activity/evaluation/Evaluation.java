...
public class Evaluation
{
	...
	{
		// commonPath="/home/gunjan/MATLAB/bin/DCU data works/July20/New_20_Aug/Copy 1 aug global 1000/";//Constant.getCommonPath();
		 ...
		 * (userid_dateOfRt_timeOfRt) for the corresponding user for correspnding recommendation time
		 */
		 ...
		 * corresponding user for corresponding recommendation time
		 */
		ArrayList<ArrayList<String>> arrayTopK = new ArrayList<ArrayList<String>>();
		/**
		 * A 2 dimensional arraylist, with rows corresponding to user and columns corresponding to recommendation times and a cell contains the actual next item (e.g. Activity
		 ...
		 * the top K items in this case are same across all RTs)
		 */
		ArrayList<ArrayList<String>> arrayBaselineOccurrence = new ArrayList<ArrayList<String>>();
		/**
		 * A 2 dimensional arraylist, with rows corresponding to user and columns corresponding to recommendation times and a cell contains the topK recommended items for the
		 * corresponding user for corresponding recommendation time, where the topK recommendations are the top K items based on duration in that user's dataset. (note: for a given
		...
		try
		{
			...
			for (String timeCategory : timeCategories)
			{
				for (int theK = theKOriginal; theK > 0; theK--)
				{
					...
					writePrecisionRecallFMeasure("BaselineOccurrence", timeCategory, theK, arrayMeta, arrayBaselineOccurrence, arrayActual);
					writePrecisionRecallFMeasure("BaselineDuration", timeCategory, theK, arrayMeta, arrayBaselineDuration, arrayActual);
				}
				
				writeReciprocalRank("Algo", timeCategory, arrayMeta, arrayTopK, arrayActual);
				writeReciprocalRank("BaselineOccurrence", timeCategory, arrayMeta, arrayBaselineOccurrence, arrayActual);
	...
	/**
	 * Calculate and write the metrics:reciprocal rank for the recommendations generated for each user for each recommendation time. (note: also write a file containing the count
	 * of number of recommendation times considered for the given parameters passsed to this method.)
	 * 
	 * @param fileNamePhrase
	 *            the name phrase used to name the file to be written
	 * @param timeCategory
	 *            time category for which metrics are to be calculated, the recommendation times to be considered are filtered based on this.
	 * @param arrayMeta
	 * @param arrayTopK
	 * @param arrayActual
	 */
	public static void writeReciprocalRank(String fileNamePhrase, String timeCategory, ArrayList<ArrayList<String>> arrayMeta,
			ArrayList<ArrayList<String>> arrayTopK, ArrayList<ArrayList<String>> arrayActual)
	{
		...
		try
		{
			 ...
			 * BufferedReader(new FileReader("/home/gunjan/dataActual.csv"));
			 */
			File fileRR = new File(commonPath + fileNamePhrase + timeCategory + "ReciprocalRank.csv");
			
			// File fileNumberOfRecommendationTimes = new File(commonPath + fileNamePhrase + timeCategory + "NumOfRecommendationTimes.csv");
			...
			// bwNumberOfRecommendationTimes = new BufferedWriter(new FileWriter(fileNumberOfRecommendationTimes.getAbsoluteFile()));
			...
			for (int i = 0; i < arrayMeta.size(); i++) // iterating over users (or rows)
			{
				...
				for (int j = 0; j < currentLineArray.size(); j++) // iterating over recommendation times (or columns)
				{
					...
					if (getTimeCategoryOfTheDay(hourOfTheDay).equalsIgnoreCase(timeCategory) || timeCategory.equals("All"))
					{
						countOfRecommendationTimesConsidered++;
						
						String actual = arrayActual.get(i).get(j);
						
						String[] topKString = arrayTopK.get(i).get(j).split("__"); // topK is of the form string: __a__b__c__d__e is of length 6... value at index 0 is empty.
						...
						// int countOfOccurence = 0; // the number of times the actual item appears in the top K recommended list. NOTE: in the current case will be always be
						// either 1 or 0.
						int rank = -99;
	...
	/**
	 * Calculate and write the metrics:precision, recall and fmeasure values, for the recommendations generated for each user for each recommendation time. (note: also write a file
	 * containing the count of number of recommendation times considered for the given parameters passsed to this method.)
	 * 
	 * @param fileNamePhrase
	 *            the name phrase used to name the file to be written
	 * @param timeCategory
			...
			ArrayList<ArrayList<String>> arrayTopK, ArrayList<ArrayList<String>> arrayActual)
	{
		...
		try
		{
			 ...
			 * BufferedReader(new FileReader("/home/gunjan/dataActual.csv"));
			 */
			File fileTopKPrecision = new File(commonPath + fileNamePhrase + timeCategory + "top" + theK + "Precision.csv");
			...
			for (int i = 0; i < arrayMeta.size(); i++) // iterating over users (or rows)
			{
				...
				for (int j = 0; j < currentLineArray.size(); j++) // iterating over recommendation times (or columns)
				{
					...
					if (getTimeCategoryOfTheDay(hourOfTheDay).equalsIgnoreCase(timeCategory) || timeCategory.equals("All"))
					{
						...
						String[] topKStrings = arrayTopK.get(i).get(j).split("__"); // topK is of the form string: __a__b__c__d__e is of length 6... value at index 0 is empty.
						...
						topKRecallVal = round((double) countOfOccurence / 1, 4); // since there is only one actual values
	...
	public static void writeMeanReciprocalRank(String fileNamePhrase, String timeCategory, int numUsers)
	{
		...
		try
		{
			...
			// File validRTCountFile = new File(commonPath + fileNamePhrase + timeCategory + "AvgReciprocalRankCountValidRT.csv");
					...
					if (lineNumber == user)
					{
						...
						{ // calculate sum
							for (int i = 0; i < rrValuesForThisUser.length; i++)
							{
								...
								if (rrValueRead >= 0) // negative P value for a given RT for given K means that we were unable to make K recommendation at that RT
								{
	...
	public static void writeAvgPrecisionsForAllKs(String fileNamePhrase, String timeCategory, int numUsers)
	{
		...
		try
		{
			...
			File validRTCountFile = new File(commonPath + fileNamePhrase + timeCategory + "AvgPrecisionCountValidRT.csv");
			validRTCountFile.delete();
			validRTCountFile.createNewFile();
			BufferedWriter bwValidRTCount = new BufferedWriter(new FileWriter(validRTCountFile.getAbsoluteFile()));
			...
			for (int user = 0; user < numUsers; user++)
			{
				...
				for (int K = theKOriginal; K > 0; K--)
				{
					...
					while ((currentLine = br.readLine()) != null)
					{
						if (lineNumber == user)
						{
							String[] pValuesForThisUserForThisK = currentLine.split(","); // get avg of all these precision values concern: splitting on empty lines gives an array
																							// of length1, also
																							// splitting on one values
																							// line gives an array of length 1
							...
							{ // calculate sum
								for (int i = 0; i < pValuesForThisUserForThisK.length; i++)
								{
									...
									// {
									// System.out.println(">>user="+user+" K="+K+" lineNumber(user)="+lineNumber+" is not null ="+pValuesForThisUserForThisK[i]);
									...
									if (pValueRead >= 0) // negative P value for a given RT for given K means that we were unable to make K recommendation at that RT
									{
	...
	public static void writeAvgFMeasuresForAllKs(String fileNamePhrase, String timeCategory, int numUsers)
	{
		...
		try
		{
			...
			File validRTCountFile = new File(commonPath + fileNamePhrase + timeCategory + "AvgFMeasureCountValidRT.csv");
			validRTCountFile.delete();
			validRTCountFile.createNewFile();
			BufferedWriter bwValidRTCount = new BufferedWriter(new FileWriter(validRTCountFile.getAbsoluteFile()));
			...
			for (int user = 0; user < numUsers; user++)
			{
				...
				for (int K = theKOriginal; K > 0; K--)
				{
					...
					while ((currentLine = br.readLine()) != null)
					{
						if (lineNumber == user)
						{
							String[] fValuesForThisUserForThisK = currentLine.split(","); // this is 1 in case of empty string
							// double[] pValuesForThisUserForThisK = new double[tokensInCurrentLine.length];
	 ...
	 * 
	 * bw.newLine(); for(int K=theKOriginal;K>0;K--) { BufferedReader br =new BufferedReader(new FileReader("/home/gunjan/top"+K+"Recall.csv"));
	 * 
	 * String currentLine;
	 * 
	 * while ((currentLine = br.readLine()) != null) { String[] rValuesForThisUserForThisK=currentLine.split(","); //double[] pValuesForThisUserForThisK = new
	 * double[tokensInCurrentLine.length];
	 * 
	 * double avgRValueForThisUserForThisK = 0; double sum=0;
	 * 
	 * for(int i=0;i<rValuesForThisUserForThisK.length;i++) { //pValuesForThisUserForThisK[i]=Double.parseDouble(tokensInCurrentLine[i]); sum = sum +
	 * Double.parseDouble(rValuesForThisUserForThisK[i]); }
	 * 
	 * avgRValueForThisUserForThisK = sum/rValuesForThisUserForThisK.length; System.out.println("Calculating avg recall (K="+K+"):"+sum+"/"+rValuesForThisUserForThisK.length); }
	 * br.close(); } bw.close(); }
	 * 
	 ...
	 * 
	 * bw.newLine(); for(int K=theKOriginal;K>0;K--) { BufferedReader br =new BufferedReader(new FileReader("/home/gunjan/top"+K+"FMeasure.csv"));
	 * 
	 * String currentLine;
	 * 
	 * while ((currentLine = br.readLine()) != null) { String[] fValuesForThisUserForThisK=currentLine.split(","); //double[] pValuesForThisUserForThisK = new
	 * double[tokensInCurrentLine.length];
	 * 
	 * double avgFValueForThisUserForThisK = 0; double sum=0;
	 * 
	 * for(int i=0;i<fValuesForThisUserForThisK.length;i++) { //pValuesForThisUserForThisK[i]=Double.parseDouble(tokensInCurrentLine[i]); sum = sum +
	 * Double.parseDouble(fValuesForThisUserForThisK[i]); }
	 * 
	 * avgFValueForThisUserForThisK = sum/fValuesForThisUserForThisK.length; System.out.println("Calculating avg recall (K="+K+"):"+sum+"/"+fValuesForThisUserForThisK.length); }
	 * br.close(); } bw.close(); }
	 * 
	 * catch (IOException e) { e.printStackTrace(); } }
	 */
