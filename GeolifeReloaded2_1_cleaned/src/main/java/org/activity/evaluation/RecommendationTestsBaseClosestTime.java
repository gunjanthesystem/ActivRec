...
public class RecommendationTestsBaseClosestTime
{
	...
	public static String commonPath;// =Constant.commonPath;//"/run/media/gunjan/OS/Users/gunjan/Documents/DCU Data Works/WorkingSet7July/";
		...
		for (String typeOfThreshold : typeOfThresholds)
		{
			...
			for (int thresholdValue : thresholdsArray)
			{
				...
				try
				{
					...
					File fileRankedRecommWithoutScore = new File(commonPath + "dataRankedRecommendationWithoutScores.csv");
					...
					// File fileCountInActivitiesGuidingRecomm= new File(commonPath+"CountInActivitiesGuidingRecomm.csv");
					...
					BufferedWriter topRecommWithDistance =
							new BufferedWriter(new FileWriter(fileRecommWithDistance.getAbsoluteFile(), true));
					
					BufferedWriter rtsInvalidWriter = new BufferedWriter(new FileWriter(fileRTsInvalid.getAbsoluteFile(), true));
					
					BufferedWriter rtsWithNoCands = new BufferedWriter(new FileWriter(fileRTsWithNoCands.getAbsoluteFile(), true));
					
					BufferedWriter rankedRecommWithScore =
							new BufferedWriter(new FileWriter(fileRankedRecommWithScore.getAbsoluteFile(), true));
					BufferedWriter rankedRecommWithoutScore =
							new BufferedWriter(new FileWriter(fileRankedRecommWithoutScore.getAbsoluteFile(), true));
					
					BufferedWriter baseLineOccurrence = new BufferedWriter(new FileWriter(fileBaseLineOccurrence.getAbsoluteFile(), true));
					BufferedWriter baseLineDuration = new BufferedWriter(new FileWriter(fileBaseLineDuration.getAbsoluteFile(), true));
					...
					BufferedWriter bwNumOfWeekendsInTraining =
							new BufferedWriter(new FileWriter(fileNumOfWeekendsInTraining.getAbsoluteFile(), true));
					
					BufferedWriter bwCountTimeCategoryOfRecomm =
							new BufferedWriter(new FileWriter(fileCountTimeCategoryOfRecomm.getAbsoluteFile(), true));
					
					BufferedWriter bwNextActInvalid = new BufferedWriter(new FileWriter(fileNextActInvalid.getAbsoluteFile(), true));
					...
					rtsInvalidWriter.write("User_ID,Date,Index_of_Activity Event,Start_Timestamp,Week_Day,Time_Categoryt\n");
					rtsWithNoCands.write(
							"User_ID,Date,Index_of_Activity Event,Start_Timestamp,Week_Day,Time_Category,Current_Activity,TotalNumOfPossibleCands,NumCandsRejectedDueToNoCurrentActivityAtNonLast,NumCandsRejectedDueToNoNextActivity\n");
					...
					// for(int userId=3;userId <4;userId++)
					{
						...
						for (Map.Entry<Date, UserDayTimeline> entry : userTestTimelines.entrySet())
						{
							...
							for (int j = 0; j < activityObjectsInThatDay.size() - 1; j++) // will not make recommendation for days which have only one activity
							{
								...
								String actRecommAtStartWithoutDistance = recommP1.getTopNextActivityNamesWithoutDistanceString();// getTopFiveRecommendedActivities();
								String actRecommAtStartWithDistance = recommP1.getTopNextActivityNamesWithDistanceString(); // its not just top five but top all
								
								String actAtRecommPoint = recommP1.getActivityAtRecomm().getActivityName();
								
								String rankedRecommAtStartWithScore = recommP1.getRankedRecommendedActivityNamesWithRankScores();
								...
								LinkedHashMap<Date, Triple<Integer, ActivityObject, Double>> distanceMapForRecomm =
										recommP1.getStartDistancesSortedMap();
										...
										.entrySet())
								{
									...
									ActivityObject endPointActivityInCandidate = entryDistance.getValue().getSecond();// dayTimelineTemp.getActivityObjectAtPosition(endPointIndexWithLeastDistanceForThisCandidate);
