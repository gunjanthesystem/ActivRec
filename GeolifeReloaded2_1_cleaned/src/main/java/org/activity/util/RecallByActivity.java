...
public class RecallByActivity
{
	...
	static final String[] activityNames = { "Others", "Commuting", "Computer", "Eating", "Exercising", "Housework", "On the Phone", "Preparing Food", "Shopping", "Socialising", "Watching TV", "Unknown" };
	...
	public static void main(String[] args)
	{
		...
		try
		{
			...
			while ((actualLine = brActual.readLine()) != null)
			{
				...
				{
					countActivityInActual.put(tokensInActualLine[i], countActivityInActual.get(tokensInActualLine[i]) + 1);
					
					// System.out.println((tokensInRecall1Line[i]));
					if (tokensInRecall1Line[i].equals("1.0"))
					{
						countActivityRecallIs1.put(tokensInActualLine[i], countActivityRecallIs1.get(tokensInActualLine[i]) + 1);
					}
				}
