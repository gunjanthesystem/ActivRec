...
/**
 * This utilises classes from the project at https://code.google.com/p/uzaygezen/ Note: we are using 0.1 version of the project as 0.2 sources files are not properly packaged in
 * their download.
 * 
 * @author gunjan
 *
...
public class HilbertCurveUtils
{
	...
	public static long getCompactHilbertCurveIndex(long lat, long lon)
	{
		...
		BitVector[] p = new BitVector[bitsPerDimension.size()];// coordinates of a point in a multi-dimensional space are represented as an array of type BitVector[]
		
		// creating emptry bitvector
		for (int i = p.length; --i >= 0;)
		{
			p[i] = BitVectorFactories.OPTIMAL.apply(bitsPerDimension.get(i));// creating emptry bitvector of bitsPerDimension.get(i), i,e, num of bits for that dimension as
																				// precision
		}
		...
		chi = BitVectorFactories.OPTIMAL.apply(chc.getSpec().sumBitsPerDimension()); // empty preallocated output variable
	...
	public static void main(String[] args)
	{
		...
		System.out.println(String.format(Locale.ROOT, "index([%s, %s])=%s", p[0].toExactLong(), p[1].toExactLong(), chi.toExactLong()));
		...
		System.out.println(
				String.format(Locale.ROOT, "indexInverse(%s)=[%s, %s]", chi.toExactLong(), p[0].toExactLong(), p[1].toExactLong()));
		...
		System.out.println(String.format(Locale.ROOT, "index([%s, %s])=%s", 12, 55, getCompactHilbertCurveIndex(12, 55)));
		
		System.out.println(String.format(Locale.ROOT, "index([%s, %s])=%s", 12, 57, getCompactHilbertCurveIndex(12, 57)));
	}
