...
public class HjorthParameters
{
	 ...
	 * "/run/media/gunjan/OS/Users/gunjan/Documents/UCD/Research/LifeloggingResearch/Sequence mining/time series/hjorth parameters/" not using the matlab implementation because it introduces a
	 * '0' when calculating derivatives to keep the length of derivative and vals same. However, in my opinion this should not be done for time series which do not start at 0. for reference of
	 * implementation: see http://www.mathworks.com/matlabcentral/fileexchange/27561-measures-of-analysis-of-time-series-toolkit--mats-/content//MATS/HjorthParameters.m
	 * 
	 * @param yt
	 */
	//
	public HjorthParameters(double[] yt)
	{
		...
		// complexity = (Math.sqrt(varianceSecondDerivative) / Math.sqrt(varianceFirstDerivative)) / (Math.sqrt(varianceFirstDerivative) / Math.sqrt(varianceYt));
		mobility = sdFirstDerivative / sdYt; // m2, spectral moment at order 2
		complexity = (sdSecondDerivative / sdFirstDerivative) / (sdFirstDerivative / sdYt); // m4, spectral moment at order 4
	}
