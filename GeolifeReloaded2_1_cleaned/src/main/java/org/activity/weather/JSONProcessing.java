...
public class JSONProcessing
{
	...
	public static void main(String args[])
	{
		...
		ArrayList<JSONArray> arrayOfHourlyArrays =
				extractHourlyDataFromForecastIOJSON("/run/media/gunjan/BoX2/GowallaSpaceSpace/Aug30/ fetchedWeatherDataAUG30.json",
						commonPath + "HourlyForDifferentFetchTime.json", true);
		ArrayList<JSONObject> arrayOfCurrWeatherObjs =
				extractCurrentlyDataFromForecastIOJSON("/run/media/gunjan/BoX2/GowallaSpaceSpace/Aug30/ fetchedWeatherDataAUG30.json",
		...
		;
		// makeTimeReadableHourlyDataFromForecastIOJSON("/run/media/gunjan/BoX2/GowallaSpaceSpace/Aug30/ fetchedWeatherDataAUG30.json",
			...
			ArrayList<JSONObject> arrayOfCurrWeatherObjs, String fileNameToWrite)
	{
		...
		for (int i = 0; i < sizeOfArrCurrObjs; i++)
		{
			...
			JSONObject currWeatherDataFromHourly = getWeatherDataFromHourlyArray(arrayOfHourlyArrays.get(i), currObj.getInt("time"));
	...
	/**
	 * Takes in an array of json objects contains hourly data for a day and return the json object correpsonding to the given query timestamp in unix epoch seconds. </br>
	 ...
	 * Extracts currently data and writes the currently weather data with lat,lon,timestamp of weather query in json format
	 * 
	 * @param fileNameToRead
	 * @param fileNameToWrite
	 */
	public static ArrayList<JSONObject> extractCurrentlyDataFromForecastIOJSON(String fileNameToRead, String fileNameToWrite)
	{
		...
		try
		{
			...
			for (String currentLine : linesRead) // each line is a json object
			{
				...
				String queryInput = "\"lat\":" + fetchedWeather.get("latitude").toString() + "," + "\"lon\":"
						+ fetchedWeather.get("longitude").toString() + "," + "\"currentTimeEpoch\":" + currentlyObj.get("time").toString()
			...
			System.out.println("---------");
			WritingToFile.writeToNewFile(result.toString(), fileNameToWrite);// commonPath + "CurrentlyForDifferentFetchTime.json");
		}
	...
	public static void extractHourlyDataFromForecastIOJSON(String fileNameToRead, String fileNameToWrite)
	{
		...
		try
		{
			...
			for (String currentLine : linesRead) // each line is a json object
			{
				...
				// indentical and is not the first line for that sample, note we have 6 lines for each lat,long,day combination. each of the six lines have different
			...
			result.append("\n]");
			System.out.println("---------");
			WritingToFile.writeToNewFile(result.toString(), fileNameToWrite);// commonPath + "HourlyForDifferentFetchTime.json");
		}
	 ...
	 * 
	 * Correctness of written data validated by looking at raw data.</br>
	 * <font color ="red">Note: identialToPrev is to compare each line (JSONObj) to previous line(JSONObj). This is relevant when running the experiment for sample to check if
	 * hourly data array is invariant of query's current timestamp for a given (lat,lon). Also, in the test sample there are 6x4 input queries (6 different timestamps in same day
	 * for each lat,lon) </font>
	 * 
	 * 
	 * @param fileNameToRead
	 * @param fileNameToWrite
	 ...
	 */
	public static ArrayList<JSONArray> extractHourlyDataFromForecastIOJSON(String fileNameToRead, String fileNameToWrite,
			boolean addISOTimestamps)
	{
		...
		try
		{
			...
			for (String currentLine : linesRead) // each line is a json object
			{
				...
				String hourlyWeatherDataString =
						"\"hourly\":{\"isIdenticalToPrevious\":" + identialToPrev + ", \"data\":" + hourlyWeatherData.toString() + "}";
	...
	// public static TreeMap<Integer, JSONObject> gettHourlyDataFromForecastIOJSON(String fileNameToRead)// , String fileNameToWrite)
	// {
	//
	// try
	// {
