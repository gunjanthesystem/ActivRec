...
public class Cluster
{
	...
	public boolean equals(Object otherObject)
	{
		...
		boolean membersAreSame = (this.members.containsAll(other.getMembers()) && (other.getMembers().containsAll(this.members)));
	...
	public boolean updateCentroid(DistancesMap distancesMap)
	{
		...
		for (int i = 0; i < allPoints.size(); i++)
		{
			...
			for (int j = 0; j < allPoints.size(); j++)
			{
				if (j != i)
				{
					sumDistances += distancesMap.getDistanceBetweenDataPoints(allPoints.get(i), allPoints.get(j));// getDistance(allPoints.get(i), allPoints.get(j));
				}
			}
		...
		if (distancesMap.getDistanceBetweenDataPoints(newCentroid, centroid) == 0) // check whether the centroid changed
		{
			...
			System.out.println("Inside cluster.updateCentroid: same centroid =" + this.getCentroid().getLabel().toString() + " but with avg dist from others = " + minAvgDistance);
		}
		else
		{
			centroidChanged = true;
			setCentroid(newCentroid);
			System.out.println("Inside cluster.updateCentroid: updated new centroid =" + this.getCentroid().getLabel().toString() + " with avg dist from others = " + minAvgDistance);
			// setCentroidInMembers(indexOfCentroidInMembers);
		}
		// System.out.println("Inside cluster.updateCentroid: updated centroid =" + this.getCentroid().toString() + " with avg dist from others = " + minAvgDistance);
		return centroidChanged;
	}
	
	// private double getDistance(DataPoint a, DataPoint b)
	// {
	// double dist = AlignmentBasedDistance.getMySimpleLevenshteinDistanceWithoutTrace(a.toString(), b.toString(), 1, 1, 2);
	...
	public boolean noDuplicates()
	{
		...
		return (hs.size() == members.size());// if the set is same size as list, this means there are no duplicates in list
	}
	...
	public String toStringUserWise()
	{
		...
		// for (int i = 0; i < members.size(); i++)
		// {
		// s += "  " + "User " + (Integer.valueOf(members.get(i).getLabel().toString()) + 1) + ";";// + members.get(i).toString() + ";";
		// // s += "  " + members.get(i).toString() + ";";
		// }
		//
		//
		...
		for (int i = 0; i < members.size(); i++)
		{
			int userIDForMember = UtilityBelt.getIndexOfUserID(Integer.valueOf(members.get(i).getLabel().toString())) + 1;
	...
	public String toStringUserWiseToPrint()
	{
		...
		// for (int i = 0; i < members.size(); i++)
		// {
		// s += "__" + "User " + (Integer.valueOf(members.get(i).getLabel().toString()) + 1);// + members.get(i).toString() + ";";
		// }
		// s += ",";
		int userIDForCentroid = UtilityBelt.getIndexOfUserID(Integer.valueOf(getCentroid().getLabel().toString())) + 1;
	...
	public double getIntraClusterVariance(DistancesMap distances)
	{
		if (members.size() == 0)
		{
			System.err.println("Error in org.activity.clustering.Cluster.getVariance() members.size()(" + members.size() + ") == 0");
			PopUps.showError("Error in org.activity.clustering.Cluster.getVariance() members.size()(" + members.size() + ") == 0");
			return -9999;
		}
		...
		else
		{
			for (int i = 0; i < members.size(); i++)
			{
				for (int j = 0; j < members.size(); j++)
				{
					if (i != j)
					{
						double distanceBetweenMembers = distances.getDistanceBetweenDataPoints(members.get(i), members.get(j));// AlignmentBasedDistance.getMySimpleLevenshteinDistanceWithoutTrace(members.get(i).toString(),
