...
public class Pair<A, B> // implements Comparable
{
	...
	/**
	 * ignore order of first and second element, check if the two items in given pair is equal to the two items in this
	 * pair
	 * 
	 * @param other
	 * @return
	...
	public boolean equalsIgnoreOrder(Object other)
	{
		if (other instanceof Pair)
		{
			...
			{
				new Exception(
						"Exception in org.activity.util.Pair.equalsIgnoreOrder(Object): first and seconds elements of current Pair are different datatypes.\n For Pair "
								+ this.toString() + " First:" + this.getFirst().getClass() + "  and second:"
								+ this.getSecond().getClass()).printStackTrace();
				return false;
			}
			else if (!((Pair) other).getFirst().getClass().equals(((Pair) other).getSecond().getClass()))
			{
				new Exception(
						"Exception in org.activity.util.Pair.equalsIgnoreOrder(Object): first and seconds elements of given Pair are different datatypes.\n For Pair "
								+ other.toString() + " First:" + ((Pair) other).getFirst().getClass() + "  and second:"
								+ ((Pair) other).getSecond().getClass()).printStackTrace();
				return false;
			}
	...
	/*
	 * @Override public int compareTo(Object target) { // TODO Auto-generated method stub Pair<A,B> pairTarget =
	 * (Pair<A,B>)target;
	 * 
