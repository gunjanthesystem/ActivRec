...
public class TimelineWithNext extends Timeline
{
	...
	int immediateNextActivityIsInvalid = -1; // -1 means 'not determined yet', '0' means 'was not invalid', '1' means 'was invalid'
			...
			Time timeAtRecomm, String userIDAtRecomm, double matchingUnitInHours)
	{
		...
		TimelineWithNext currentTimeline = new TimelineWithNext(activityObjectsInCurrentTimeline, nextValidActivityObject);
	...
	/**
	 * Fetches the current timeline from the given longer timeline from the recommendation point back until the matching unit count Activity Objects.
	 * 
	 * @param longerTimeline
	 *            the timelines (test timeline) from which the current timeline is to be extracted
	 * @param dateAtRecomm
			...
			Time timeAtRecomm, String userIDAtRecomm, double matchingUnitInCountsD)
	{
		...
		System.out.println("Adjusted MU:" + matchingUnitInCounts);// + " Current Timeline:" + currentTimeline.getActivityObjectNamesWithTimestampsInSequence());
