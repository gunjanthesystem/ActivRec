...
public class Timeline
{
	...
	public Timeline(ArrayList<ActivityObject> activityObjects)
	{
		...
		/*
		 * if(activityObjects.size() ==0 ) { System.err.println("Error in creating Timeline: Empty Activity Objects provided"); System.exit(5); }
		 * 
		 * else
		 */
		{
			...
			/*
			 * if(isSameDay(activityObjectsInTimeline) == true) { this.dateID=activityObjectsInTimeline.get(0).getDimensionIDValue("Date_ID");
			 * 
			 * this.dayName=activityObjectsInTimeline.get(0).getDimensionAttributeValue("Date_Dimension", "Week_Day").toString();
			 * 
		...
		{
			System.err.println("Error: in Timeline(ArrayList<ActivityObject> activityObjects), CHRONOLOGY NOT PRESERVED");
		}
	...
	/**
	 * Creates a Timeline consisting of the Activity Objects from the given LinkedHashMap of UserDayTimelines thus essentially converts mulitple day timelines into a single
	 * continuous single timeline
	 * 
	 * @param dayTimelines
	...
	public Timeline(LinkedHashMap<Date, UserDayTimeline> dayTimelines)
	{
		...
		if (!isChronological(activityObjects))
		{
			System.err.println("Error: in Timeline(LinkedHashMap<Date,UserDayTimeline> dayTimelines), CHRONOLOGY NOT PRESERVED");
		}
		long lt = System.currentTimeMillis();
		
		if (Constant.verboseTimelineCleaning)
			System.out.println("Creating timelines for " + dayTimelines.size() + " daytimelines  takes " + (lt - dt) / 1000 + " secs");
	}
	
	/**
	 * Creates a Timeline consisting of the Activity Objects from the given LinkedHashMap of UserDayTimelines thus essentially converts mulitple day timelines into a single
	 * continuous single timeline with given ID for the timeline
	 * 
	 * @param dayTimelines
	 * @param timelineID
	...
	public boolean isChronological(ArrayList<ActivityObject> listToCheck)
	{
		...
		{
			if (listToCheck.get(i).getStartTimestamp().after(listToCheck.get(i + 1).getStartTimestamp())) // one activity object's starttimestamp is after the next activity
																											// object's
																											// starttimestamp,
																											// implying the breaking
																											// of chronoligcal order
			{
	...
	public int countNumberOfValidDistinctActivities()
	{
		...
		for (int i = 0; i < activityObjectsInTimeline.size(); i++)
		{
			...
			// ((activityObjectsInTimeline.get(i).getActivityName().equalsIgnoreCase("Unknown") || activityObjectsInTimeline.get(i).getActivityName().equalsIgnoreCase("Others"))
			// ==false)
			{
	...
	public ActivityObject getNextValidActivityAfterActivityAtThisPosition(int indexOfActivityObject)
	{
		...
		// $$System.out.print("\t getNextValidActivityAfterActivityAtThisPosition(): Index of activity object to look after is " + indexOfActivityObject +
		...
		if (nextValidActivityObject.getActivityName().equals(activityObjectsInTimeline.get(indexOfActivityObject).getActivityName()))
		{
			if (Constant.verbose)
			{
				System.err.println("\n\t Warning: Next Valid activity has same name as current activity (for timelineID:" + timelineID
						+ ") Activity Name=" + activityObjectsInTimeline.get(indexOfActivityObject).getActivityName());
		...
		/*
		 * if(nextValidActivityObject==null) WHY DEAD CODE { System.err.println("Warning: No next valid activity after this index in the given timeline");
		 * System.err.println("\tThe timeline is:"+this.getActivityObjectNamesInSequence()); System.err.println("\tEnd index index is:"+indexOfActivityObject); }
		 */
	...
	public static boolean isNoValidActivityAfterIt(int activityObjectIndex, Timeline givenTimeline)
	{
		...
		for (int i = activityObjectIndex + 1; i < objectsInGivenTimeline.size(); i++)
		{
			...
			// if((objectsInGivenTimeline.get(i).getActivityName().equalsIgnoreCase("Unknown") || objectsInGivenTimeline.get(i).getActivityName().equalsIgnoreCase("Others"))
			// ==false)
			{
	...
	public String getActivityObjectsAsStringCode()
	{
		...
		for (int i = 0; i < activityObjectsInTimeline.size(); i++)
		{
			...
			stringCodeForTimeline.append(StringCode.getStringCodeFromActivityName(activityName)); // Character.toString ((char)(activityID+65)); //getting the ascii code for
																									// (activity id+65)
		}
	...
	public ArrayList<Integer> getActivityObjectsAsSequenceOfNumericCodes()
	{
		...
		for (int i = 0; i < activityObjectsInTimeline.size(); i++)
		{
			...
			stringCodeForTimeline += StringCode.getStringCodeFromActivityName(activityName); // Character.toString ((char)(activityID+65)); //getting the ascii code for (activity
																								// id+65)
		}
	...
	public boolean hasAValidActivityAfterFirstOccurrenceOfThisActivity(ActivityObject activityToCheck)
	{
		...
		try
		{
			...
			if (indexOfFirstOccurrence < this.activityObjectsInTimeline.size() - 1) // not the last activity of the timeline
			{
	...
	/**
	 * Returns Activity Objects falling between given start time and end time (all activity objects intersecting this starttime-endtime interval is included. note: if you want to
	 * find activity object at time t, make start and end timestamp equal upto the resolution (usually seconds) required.
	 * 
	...
	public ArrayList<ActivityObject> getActivityObjectsBetweenTime(Timestamp startTimestampC, Timestamp endTimestampC)
	{
		...
		{
			/*
			 * $$30Seplong intersectionOfActivityObjectAndIntervalInSeconds= activityObjectsInTimeline.get(i).intersectingIntervalInSeconds(startTimestampC, endTimestampC);
			 * if(intersectionOfActivityObjectAndIntervalInSeconds >0) { activityObjectsIn.add(activityObjectsInTimeline.get(i)); } $$30Sep
			 */
			// 30Sep addition
		...
		if (Constant.verbose)
		{
			System.out.print("\t Inside getActivityObjectsBetweenTime: activity objects inside " + startTimestampC + " and " + endTimestampC
					+ " are: ");
	...
	public int getIndexOfActivityObjectsAtTime(Timestamp timestampC)
	{
		...
		if (index == -99)
		{
			System.err.println("Error in  getIndexOfActivityObjectsAtTime(): No Activity object at this timestamp:" + timestampC);
			new Exception("Error in  getIndexOfActivityObjectsAtTime(): No Activity object at this timestamp:" + timestampC);
		}
		return index;
	}
	...
	public ArrayList<ActivityObject> getActivityObjectsStartingOnBeforeTime(Timestamp startTimestampC)
	{
		...
		/*
		 * System.out.println("Intersect: The activity objects inside "+startTimestampC+ " and "+endTimestampC+" are "); for(int i=0;i<activityObjectsIn.size();i++) {
		 * System.out.print(activityObjectsIn.get(i).getActivityName()); }
		 */
		return activityObjectsIn;
	}
	 ...
	 * @return
	 */
	public boolean isSameDay()// ArrayList<ActivityObject> activityObjectsInDay) // checks whether all activity objects in the timeline are of the same day or not
	{
		boolean sane = true;
		...
		if (this.activityObjectsInTimeline.size() > 0)
		{
			...
			{
				if (date.equals((Date) activityObjectsInTimeline.get(i).getDimensionAttributeValue("Date_Dimension", "Date")) == false)
				{
					sane = false;
				}
			}
		...
		if (!sane)
		{
			System.err.println("Timeline  contains ActivityObjects from more than one day"); /* with Date_ID:"+ dateID+" */
			// System.exit(3);
		}
		return sane;
	}
	...
	public LongSummaryStatistics getDurationStats()
	{
		...
		summary = allAOs.stream().collect(Collectors.summarizingLong(ao -> ao.getDurationInSeconds()));// .map(ao -> ao.getDurationInSeconds()).
