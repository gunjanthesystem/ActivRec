...
public class TrajectoryEntry implements Serializable
{
	...
	private ArrayList<Timestamp> timestamps; // timestamps of data points inside it. (useful in cases when this is a
												// merged data entry)
	...
	public TrajectoryEntry(String mode, ArrayList<String> lat, ArrayList<String> lon, ArrayList<String> alt,
			ArrayList<String> trajectoryID, ArrayList<Timestamp> timestamps, Timestamp timestamp,
			long differenceWithNextInSeconds, long durationInSeconds, int breakOverDaysCount)
	{
		super();
		this.mode = mode;
		this.lat = lat;
	...
	{// "timestamp,
		...
		return timestamp.toGMTString() + " , " + endTimestampString + ", " + mode + ", "
				...
				+ ", " + getNumberOfDistinctTrajectoryIDs() + "," + getNumberOfTrajectoryIDs()
	...
	{// "timestamp,
		...
		return timestamp.toGMTString() + " , " + endTimestampString + ", " + mode + ", "
				+ this.differenceWithNextInSeconds + ", " + this.durationInSeconds + ", " + this.breakOverDaysCount
				+ ", " + getNumberOfDistinctTrajectoryIDs() + "," + getNumberOfTrajectoryIDs() + ","
				+ StringUtils.toStringCompactWithoutCount(this.trajectoryID).replaceAll(",", "_") + ", "
				+ lat.toString().replaceAll(",", "_") + ", " + lon.toString().replaceAll(",", "_") + ", "
				+ alt.toString().replaceAll(",", "_");
	...
	public static TrajectoryEntry mergeWithoutQuestion(TrajectoryEntry tleA, TrajectoryEntry tleB) throws Exception
	{
		...
		// System.out.println("Inside mergeWithoutQuestion: merging teA with " + tleA.getTrajectoryID().size() + "
		// trajIDs and teB with "
		...
		newTle = new TrajectoryEntry("", tleA.getLatitude(), tleA.getLongitude(), tleA.getAltitude(),
				tleA.getTrajectoryID(), tleA.getTimestamps(), tleA.getTimestamps().get(0),
				newDifferenceWithNextInSeconds, newDurationInSeconds, -99); // note break over days count is -99 as it
																			// is
																			// not relevant here.
		// making a copy
