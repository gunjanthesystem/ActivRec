...
// Value rFunction = context.eval("R", "function(table) { " + " table <- as.data.frame(table);"
// + " cat('The whole data frame printed in R:\n');" + " print(table);" + " cat('---------\n\n');" +
//
// " cat('Filter out users with ID>2:\n');" + " print(table[table$id > 2,]);" + " cat('---------\n\n');"
// +
//
// " cat('How many users like Java: ');" + " cat(nrow(table[table$language == 'Java',]), '\n');" + "}");
// User[] data = getUsers();
...
// {
// return new User[] { new User(1, "Florian", "Python"), new User(2, "Lukas", "R"), new User(3, "Mila", "Java"),
// new User(4, "Paley", "Coq"), new User(5, "Stepan", "C#"), new User(6, "Tomas", "Java"),
