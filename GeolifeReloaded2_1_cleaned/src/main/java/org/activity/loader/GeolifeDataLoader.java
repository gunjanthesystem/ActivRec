...
public class GeolifeDataLoader
{
	...
	/*
	 * static final String[] activityNames= {"Others", "Commuting", "Computer", "Eating", "Exercising", "Housework", "On the Phone", "Preparing Food", "Shopping", "Socialising",
	 * "Watching TV", "Unknown"};
	 * 
	...
	static String commonPath =
			"/run/media/gunjan/HOME/gunjan/Geolife Data Works/21April2016AllUsersDataGenerationUMAPAgainCorrectedTimeZone/";// "/run/media/gunjan/Space/GUNJAN/GeolifeSpaceSpace/May18AllUsersDataGeneration2/";
	...
	public static void main(String[] args)
	{
		try
		{
			...
			// Constant.setUserIDs();// Constant.setInvalidNames(); // Constant.setActivityNames(); // Constant.setCommonPath(commonPath);
			...
			PrintStream loadLogStream = WritingToFile.redirectConsoleOutput(Constant.getCommonPath() + dataLoadingLogFilename);
			
			// deserialiser for one map data
			// mapForAllDataMergedPlusDuration =
			// (LinkedHashMap<String, TreeMap<Timestamp, TrajectoryEntry>>) (Serializer.deSerializeThis(Constant.getCommonPath()
			...
			mapForAllDataMergedPlusDuration =
					(LinkedHashMap<String, TreeMap<Timestamp, TrajectoryEntry>>) (Serializer.deserialiseAndConsolidateMaps(
							Constant.getCommonPath() + nameForDataMapSerialised, 0, 65, 5));
			
			// traverseMapForAllData(testSerializer);;
			
			// mapForAllDataMergedPlusDuration=replaceDataEntry(mapForAllDataMergedPlusDuration,"Not Available","Others");
			...
			for (Map.Entry<String, TreeMap<Timestamp, TrajectoryEntry>> entry : mapForAllDataMergedPlusDuration.entrySet())
			{
				...
				for (Map.Entry<Timestamp, TrajectoryEntry> dataForAUser : entry.getValue().entrySet())
				{
					...
					{
						// ## System.out.println("inserting in activity dimension:"+activityId+" "+activityName+" "+activityCategory);
						ConnectDatabase.insertIntoActivityDimension(activityId, activityName, activityCategory); // only arund 13 insertions, hence batch update is not necessary
					...
					// Timestamp endTimestamp = new Timestamp(dataForAUser.getKey().getTime()+ getDurationInSecondsFromDataEntry(dataForAUser.getValue())*1000);
					...
					Timestamp endTimestamp = new Timestamp(startTimestamp.getTime() + (duration * 1000) - 1000); // decrementing 1 second to keep consecutive activities separated
																													// by 1 seconds
					int endHour = endTimestamp.getHours();
					int endMinute = endTimestamp.getMinutes();
					int endSecond = endTimestamp.getSeconds();
					
					// timeId= //timeId+1; (iiWAS version) .... (the version causing error: startHour+startMinute+startSecond+endHour+endMinute+endSecond; )
					
					// String timeIdString = Integer.toString(startHour)+Integer.toString(startMinute)+Integer.toString(startSecond)
					...
					timeId = timeId + 1;// Long.parseLong(Integer.toString(startHour) + Integer.toString(startMinute) + Integer.toString(startSecond) + Integer.toString(endHour) +
					...
					{
						System.err.println("Error in data loading wrt: starttime-endtime != duration,  starttime=" + startTimestamp
								+ " endtime=" + endTimestamp + " duration=" + duration);
						System.err.println("(endTimestamp.getTime()-startTimestamp.getTime()/1000) = "
								+ ((endTimestamp.getTime() - startTimestamp.getTime()) / 1000));// +" endtime"+endTimestamp+" duration="+duration);
					}
					
					if (((endTimestamp.getTime() < startTimestamp.getTime())))
					{
						System.err.println("endTimestamp.getTime() <startTimestamp.getTime()  starttimestamp=" + startTimestamp
								+ " endtimestamp=" + endTimestamp + " duration=" + duration);
					}
					
					// else
					// System.out.println("\nstarttime="+startTimestamp+" endtime="+endTimestamp+" duration="+duration+"  endHourminutesseconds="+endHour+":"+endMinute+":"+endSecond);
					...
					dateId = dateId + 1;// Long.parseLong(Integer.toString(day) + Integer.toString(month) + Integer.toString(year) + Integer.toString(endday) +
					...
					FlatActivityLogEntry activityLogEntry = new FlatActivityLogEntry(); // represents a row of all combined table
					...
					activityLogEntry.setLatitudes(latitudes.substring(1, latitudes.length() - 1));// truncating opening and closing braces
					activityLogEntry.setLongitudes(longitudes.substring(1, longitudes.length() - 1));// truncating opening and closing braces
					activityLogEntry.setAltitudes(altitudes.substring(1, altitudes.length() - 1));// truncating opening and closing braces
					...
					activityLogEntry.setEndAltitude(te.getEndAlt());// te.getAltitude().get(te.getAltitude().size() - 1));
					...
					// for current implemenation this is always unique..hence for each entry we need to insert a location entry.
					if (locationId == 1)
					{
			...
			WritingToFile
					.writeArrayListFlatActivityLogEntry(
							listOfActivityEntries,
							"FlatActivityLogEntries",
			...
			LinkedHashMap<String, TreeMap<Timestamp, String>> map)
	{
		...
		for (Map.Entry<String, TreeMap<Timestamp, String>> entry : map.entrySet())
		{
			...
			for (int i = 0; i < treeMapArrayList.size() - 1; i++)
			{
				...
				long differenceBeforeNextStarts = nextTimestamp.getTime() / 1000 - (currentTimestamp.getTime() / 1000 + currentDuration);
				
				System.out.println(">> the time difference before next starts is: " + differenceBeforeNextStarts);
				if (differenceBeforeNextStarts > 0)
					System.out.println("GAP found in time continuum");
				
			}
			
			/*
			 * for (Map.Entry<Timestamp,String> entryTree : entry.getValue().entrySet()) { newTreeMap.put(entryTree.getKey(),entryTree.getValue().replaceAll(oldString, newString));
			 * 
			 * 
			 * }
			 * 
			 * if(entry.getValue().size() != newTreeMap.size()) System.err.println("Error in replace Data Entry: size mismatch of treemap");
			 * 
		...
		/*
		 * if(map.size() != newMap.size()) System.err.println("Error in replace Data Entry: size mismatch of linked hash map");
		 */
		return newMap;
	}
	 ...
	 * 
	 * if(userId==-99) { System.err.println("Error: inside getUserIdFromName - Cannot find User ID for User = "+userName); } return userId; }
	 * 
	 ...
	 * 
	 * if(activityId==-99) { System.err.println("Error: inside getActivityIdFromActivityName - Cannot find Activity ID for Activity Name = "+activityName); } return activityId; }
	 */
	...
	public static String getWeekOfYear(Timestamp timestamp)
	{
		...
		try
		{
			weekOfYear = ConnectDatabase.getSQLStringResultSingleColumn("SELECT WEEKOFYEAR('" + timestamp + "');").get(0);
		}
		catch (Exception e)
		{
			e.printStackTrace();
	...
	public static LinkedHashMap<String, TreeMap<Timestamp, String>> replaceDataEntry(LinkedHashMap<String, TreeMap<Timestamp, String>> map,
	...
	/*
	 * public static LinkedHashMap<String, TreeMap<Timestamp,String>> fillWithUnknown(LinkedHashMap<String, TreeMap<Timestamp,String>> map) { LinkedHashMap<String,
	 * TreeMap<Timestamp,String>> newMap= new LinkedHashMap<String, TreeMap<Timestamp,String>>();
	 * 
	 * for (Map.Entry<String, TreeMap<Timestamp,String>> entry : map.entrySet()) {
	 * 
	 * 
	 ...
	 * 
	 * for (Map.Entry<Timestamp,String> entryTree : entry.getValue().entrySet()) { //newTreeMap.put(entryTree.getKey(),entryTree.getValue().replaceAll(oldString, newString));
	 * Timestamp startTimestamp=entryTree.getKey();
	 * 
	 * 
	 * String[] splitted=entryTree.getValue().split(Pattern.quote("||")); long durationInSeconds = Long.valueOf(splitted[1]);
	 * 
	 * intervalMap.put(new TimeInterval(entryTree.getKey(),durationInSeconds), entryTree.getValue()); }
	 * 
	 * if(entry.getValue().size() != intervalMap.size()) System.err.println("Error in replace Data Entry: size mismatch of treemap");
	 * 
	 * newMap.put(entry.getKey(), newTreeMap); }
	 * 
	 * if(map.size() != newMap.size()) System.err.println("Error in replace Data Entry: size mismatch of linked hash map");
	...
	/*
	 * public static String getActivityName(int activityId) { String activityName=" "; switch(activityId) { case 0: activityName= "Commuting"; break; ////////// categories in DCU
	 * data Version 1 case 1: activityName= "Working"; break; case 2: activityName= "Socialising"; break;///////// case 3: activityName= "Computer"; break;/////////// case 4:
	 * activityName= "WatchingTV"; break;////////// case 5: activityName= "Shopping"; break;///////////// case 6: activityName= "On Phone"; break;/////////// case 7: activityName=
	 * "Relaxing"; break; case 8: activityName= "Meditating"; break; case 9: activityName= "Taking care of children"; break; case 10: activityName= "Preparing Food";
	 * break;/////////// case 11: activityName= "Housework"; break;///////// case 12: activityName= "Eating"; break;/////////// case 13: activityName= "Nap/Resting"; break; case
	 * 14: activityName= "Exercising"; break;/////////// case 15: activityName= "Uncategorised"; break; ///// this is the 'badImages default: activityName= "Others"; break; }
	 * return activityName; }
	 * 
	 * public static String getActivityCategory(String activityName) { int id=0;
	 * 
	 * switch(activityName) { case "Commuting": id=0 ; break; case "Working": id=1 ; break; case "Socialising": id=2 ; break; case "Computer": id=3 ; break; case "WatchingTV": id=4
	 * ; break; case "Shopping": id=5 ; break; case "On Phone": id=6 ; break; case "Relaxing": id=7 ; break; case "Meditating": id=8 ; break; case "Taking care of children": id=9 ;
	 * break; case "Preparing Food": id=10 ; break; case "Housework": id=11 ; break; case "Eating": id=12 ; break; case "Nap/Resting": id=13 ; break; case "Exercising": id=14 ;
	 * break; case "Uncategorised": id=15; break; ///// this is the 'badImages default: id= 99; break; }
	 * 
	...
	public static String getActivityCategoryFromActivityName(String activityName)
	{
		...
		/*
		 * DCU {"badImages", 0/// "Commuting",1/////////////// "Computer", 2//////////// "Eating", 3 "Exercising", 4 "Housework", 5 "On the Phone", 6 "Preparing Food", 7
		 * "Shopping", 8 "Socialising", 9 "Watching TV"}; 10
		 */
		...
		/*
		 * switch(activityId%3) { case 0: activityCategory="category 1"; break; //commuting, computer, on phone, taking care, eating, case 1: activityCategory="category 2"; break;
		 * //working, watchingtv. relaxing, preparing food, nap/resting case 2: activityCategory="category 3"; break; //socialising, shopping, meditating, housework, exercising
		 * default: activityCategory="others"; break; }
		 */
	...
	public static String getActivityCategory(int activityId)
	{
		...
		/*
		 * DCU {"badImages", 0 "Commuting",1 "Computer", 2 "Eating", 3 "Exercising", 4 "Housework", 5 "On the Phone", 6 "Preparing Food", 7 "Shopping", 8 "Socialising", 9
		 * "Watching TV"}; 10
		 */
	 ...
	 * 
	 * for(int iterator=0;iterator<categoryNames.length;iterator++) { if(containsText(path+userName+"_"+categoryNames[iterator]+".txt", imageName)) {
	 * category=categoryNames[iterator]; return category; } } return category; }
	 */
	 ...
	 * 
	 * try { Class.forName(JDBC_DRIVER); System.out.println("Connecting to a selected database for insertion in activity_fact_table..."); conn = DriverManager.getConnection(DB_URL,
	 * USER, PASS); System.out.println("Connected database successfully..."); String sqlInsertion = "INSERT INTO "+databaseName+"." +databaseTableName+ " VALUES (?,?,?,?,?,?,?)";
	 * 
	 * stmt = conn.prepareStatement(sqlInsertion);
	 * 
	 * for(int userIterator=0;userIterator<userNames.length;userIterator++) {
	 * 
	 * for(int activityIterator=0; activityIterator< categoryNames.length;activityIterator++) { try { stmt.setString(1,"Dummy"); stmt.setString(2,userNames[userIterator]);
	 * stmt.setTimestamp(3,null); stmt.setString(4,categoryNames[activityIterator]); stmt.setString(5,null); stmt.setString(6,null); stmt.setInt(7,0);
	 * 
	 ...
	 * stmt = conn.prepareStatement(sqlInsertion);
	 * 
	 * for(int userIterator=0;userIterator<userNames.length;userIterator++) { String imageListFileName=path+userNames[userIterator]+"_JPGFiles.txt";
	 * 
	 ...
	 * 
	 * 
	 * if((currentImageName = br.readLine()) == null) { System.out.println("ERROR: User:"+userNames[userIterator]+" file:"+imageListFileName+" is empty"); br.close();return ; }
	 * 
	 * while ((currentImageName = br.readLine()) != null) { //getCategory(userNames[userIterator],currentImageName);
	 * 
	 * currentImageName= currentImageName.substring(0, currentImageName.length()-1); //removing the last comma Timestamp timestamp=getTimestamp(currentImageName);
	 * 
	 * String dayName=getSQLStringResultSingleColumn("SELECT DAYNAME('"+timestamp+"');").get(0); stmt.setString(1,currentImageName); stmt.setString(2,userNames[userIterator]);
	 * stmt.setTimestamp(3,getTimestamp(currentImageName)); stmt.setString(4,getCategory(userNames[userIterator],currentImageName)); stmt.setString(5,dayName);
	 ...
	 * 
	 * 
	 * public static ArrayList<String> getSQLStringResultSingleColumn(String query) throws SQLException { Connection conn = null; PreparedStatement stmt = null; ResultSet resultset
	 * = null; ArrayList<String> resultStrings=new ArrayList<String>();
	 * 
	 * try { Class.forName(JDBC_DRIVER); System.out.println("Connecting to a selected database for getSQLStringResult()..."); conn = DriverManager.getConnection(DB_URL, USER,
	 * PASS); System.out.println("Connected database successfully..."); String sqlQuery = query; stmt = conn.prepareStatement(sqlQuery); resultset = stmt.executeQuery();
	 ...
	 * 
	 * int userAge= getUserAge(userId); String ageCategory=getAgeCategory(userAge); String profession=getProfession(userId); String personality=getPersonality(profession);
	 * 
	 * ConnectDatabase.insertIntoUserDimension(userId,userName,userAge,personality,profession,ageCategory);
	 * 
	 ...
	 * 
	 * 
	 * if((currentImageName = br.readLine()) == null) { System.out.println("ERROR: User:"+userName+" file:"+imageListFileName+" is empty"); br.close();return ; }
	 * 
	 * while ((currentImageName = br.readLine()) != null) { //getCategory(userNames[userIterator],currentImageName);
	 * 
	 ...
	 * 
	 * 
	 * dateId=Integer.parseInt( Integer.toString(timestamp.getDate())+ Integer.toString(timestamp.getMonth())+ Integer.toString(timestamp.getYear()) );
	 * 
	 * String activityName=getCategory(userName,currentImageName); // here get Category fetches the activity name, .i.e activity category means activity name in this case String
	 * activityCategory=getActivityCategory(activityName); int activityId=getActivityid(activityName);
	 * 
	 * if(!(uniqueActivityIds.contains(activityId))) { ConnectDatabase.insertIntoActivityDimension(activityId,activityName,activityCategory); uniqueActivityIds.add(activityId); }
	 * 
	 * LocationObject locationObject=LocationObject.getLocationObjectInstance(userId,99); // dummy, not available in data currently
	 * 
	 * timeId=Integer.parseInt( Integer.toString(timestamp.getHours())+ Integer.toString(timestamp.getMinutes())+ Integer.toString(endHour)+ Integer.toString(endMinute));
	 * 
	 * stmt.setString(1,currentImageName); stmt.setString(2,userNames[userIterator]); stmt.setTimestamp(3,getTimestamp(currentImageName));
	 * stmt.setString(4,getCategory(userNames[userIterator],currentImageName)); stmt.setString(5,dayName); stmt.setDate(6,getDate(currentImageName)); stmt.setInt(7,1);
	 * 
	 * stmt.executeUpdate();
	 * 
	 * 
