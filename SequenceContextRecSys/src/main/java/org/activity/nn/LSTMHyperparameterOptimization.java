...
/**
 * This is a basic hyperparameter optimization example using Arbiter to conduct random search on two network
 * hyperparameters. The two hyperparameters are learning rate and layer size, and the search is conducted for a simple
 * multi-layer perceptron on MNIST data.
 *
 * Note that this example is set up to use Arbiter's UI: http://localhost:9000/arbiter
 *
 * @author Alex Black
 */
public class LSTMHyperparameterOptimization
{
	...
	public static void main(String[] args) throws Exception
	{
		...
		int minLayerSize = 16;
		int maxLayerSize = 256;
		double l2Coeff = 0.0001;
		int numOfTrainingEpochs = 2;
		int batchSize = 64;

		new LSTMHyperparameterOptimization(minLearningRate, maxLearningRate, minLayerSize, maxLayerSize, l2Coeff,
				numOfTrainingEpochs, batchSize);
	}

	public LSTMHyperparameterOptimization(double minLearningRate, double maxLearningRate, int minLayerSize,
			int maxLayerSize, double l2RegularisationCoeff, int numOfTrainingEpochs, int batchSize)
			throws IOException, InterruptedException
	{
		// First: Set up the hyperparameter configuration space. This is like a MultiLayerConfiguration, but can have
		// either fixed values or values to optimize, for each hyperparameter

		// Values will be generated uniformly at random between 0.0001 and 0.1 (inclusive)
		ParameterSpace<Double> learningRateHyperparam = new ContinuousParameterSpace(minLearningRate, maxLearningRate);
		// Integer values will be generated uniformly at random between 16 and 256 (inclusive)
		ParameterSpace<Integer> layerSizeHyperparam = new IntegerParameterSpace(minLayerSize, maxLayerSize);

		Builder hyperparamSpaceBuilder = new MultiLayerSpace.Builder();

		///////// These next few options: fixed values for all models
		hyperparamSpaceBuilder.seed(123);
		hyperparamSpaceBuilder.l2(l2RegularisationCoeff);// 0.001);
		hyperparamSpaceBuilder.weightInit(WeightInit.XAVIER);
		////////

		////////// Learning rate hyperparameter: search over different values, applied to all models
		hyperparamSpaceBuilder.updater(new RmsPropSpace(learningRateHyperparam));
		////////

		hyperparamSpaceBuilder.addLayer(new DenseLayerSpace.Builder()
				// Fixed values for this layer:
				.nIn(784) // Fixed input: 28x28=784 pixels for MNIST
				.activation(Activation.LEAKYRELU)
				// One hyperparameter to infer: layer size
				.nOut(layerSizeHyperparam).build());
		hyperparamSpaceBuilder.addLayer(new OutputLayerSpace.Builder().nOut(10).activation(Activation.SOFTMAX)
				.lossFunction(LossFunctions.LossFunction.MCXENT).build());

		MultiLayerSpace hyperparameterSpace = hyperparamSpaceBuilder.build();

		// Now: We need to define a few configuration options
		// (a) How are we going to generate candidates? (random search or grid search)
		// Alternatively: new GridSearchCandidateGenerator<>(hyperparameterSpace, 5,
		...
		// int nTrainEpochs = 2;int batchSize = 64;
		DataProvider dataProvider = new ExampleDataProvider(numOfTrainingEpochs, batchSize);
		...
		OptimizationConfiguration configuration = new OptimizationConfiguration.Builder()
				.candidateGenerator(candidateGenerator).dataProvider(dataProvider).modelSaver(modelSaver)
