...
public class AttributeControlObject
{
	...
	public AttributeControlObject(String name, String datatype, boolean hasOperatorSelection, String hierarchy,
			String belongsToDimension)
	{
		this.name = name.trim();
	 ...
	 * 
	 * String htmlString=""; if(targetAttributeControlObject.getDatatype().equalsIgnoreCase("Varchar")) {
	 * htmlString="<div class=\"row marginise  "+
	 * targetAttributeControlObject.getName()+"AggregationAttributeSelectionRow\""+ // we have a missing > here but UI
	 * butting aligns better if its missing, need to correct it later "<div class=\"btn-group marginise\">"+
	 ...
	 * "VarcharSelection\">"+
	 * //HTMLStrings.getHTMLOptionValueVarchar(targetAttributeControlObject.getName(),targetAttributeControlObject.
	 * getBelongsToDimension())+ // "</select>"+ " <button type='button' id = '"+targetAttributeControlObject.getName()
	 ...
	 * " $('#"+targetAttributeControlObject.getName()+"DeleteButton').click(function()"+ "{"+
	 * "$((this).parentNode).html('');"+ // $('#section1').removeClass().addClass('active breadcrumb')+
	 * "$('#"+targetAttributeControlObject.getName()+"AggregationMenuOption').removeClass()"+
	 * //"alert($((this).parentNode).className);"+ "});"+ "</script>"; //%%
	...
	{
		AttributeControlObject targetAttributeControlObject = MetaBase
				.identifyAttribute(alias_UI_Name.replace(' ', '_'));

		String htmlString = "";
		if (targetAttributeControlObject.getDatatype().equalsIgnoreCase("Varchar"))
		{
			htmlString = "<div class=\" marginise  " + targetAttributeControlObject.getName()
					+ "AttributeSelectionRow\"" + // we have a missing > here but UI butting aligns better
													// if its missing, need to correct it later
					"<div class=\"btn-group marginise\">"
					+ "<button type=\"button\" class=\"btn btn-info disabled flat-border \">"
					+ targetAttributeControlObject.getName().replace('_', ' ') + "</button>"
					+ "<select class=\"multiSelect\" multiple=\"multiple\" id=\""
					+ targetAttributeControlObject.getName() + "VarcharSelection\">"
		...
		{
			// controlObjects.add(new
			// AttributeControlObject(targetAttributeControlObject.getName(),targetAttributeControlObject.getDatatype(),true));
			htmlString = "<div class=\"marginise  " + targetAttributeControlObject.getName() + "AttributeSelectionRow\""
					+ // we have a missing > here but UI butting aligns better
						// if its missing, need to correct it later
					...
					+ targetAttributeControlObject.getName() + "OperatorSelection\" id=\""
					+ targetAttributeControlObject.getName() + "OperatorSelection\"" + " role=\"menu\">"
					+ "<li><a href=\"#\">=</a></li>" + "<li><a href=\"#\">>=</a></li>" + "<li><a href=\"#\"><=</a></li>"
					+ "<li><a href=\"#\">!=</a></li>" + "</ul>" + "</span>" +

					"<span class=\"input-group date col-xs-2\" style=\"width: 130px;\" id=\""
					+ targetAttributeControlObject.getName() + "TDateSelection\">"
					+ "<span class=\"input-group-addon\">" + "<span class=\"glyphicon glyphicon-time\"></span>"
					+ "</span>" + "<input type=\"text\" class=\"form-control\" placeholder=\"mm/dd/yyyy\" />"
					 ...
					 * 
					 * htmlString="<div class=\"row marginise   "+
					 * targetAttributeControlObject.getName()+"AttributeSelectionRow\""+ // we have a missing > here but
					 * UI butting aligns better if its missing, need to correct it later
					 * "<div class=\"btn-group marginise\">"+ "<span class=\"btn-group\">"+
					 * "<button type=\"button\" class=\"btn flat-border btn-info disabled\">"+
					 * targetAttributeControlObject.getName().replace('_',' ')+ "</button>"+
					 * 
					 * "<span class=\"btn-group\">"+
					 * "<button type=\"button\" class=\"btn flat-border btn-default dropdown-toggle "+
					 * targetAttributeControlObject.getName()+"OperatorDisplay \" data-toggle=\"dropdown\">"+
					 ...
					 * 
					 * "<span class=\"input-group date col-xs-2\" style=\"width: 2 0px;\" id=\""
					 * +targetAttributeControlObject.getName()+"TDateSelection\">"+ //notes: there is a T added before
					 * DateSelection because apparently Dateselection gives an error of not disable time while TDate
					 * Selection works. "<span class=\"input-group-addon\">"+
					 * "<span class=\"glyphicon glyphicon-time\"></span>"+ "</span>"+
					 * "<input type=\"text\" class=\"form-control\" placeholder=\"mm/dd/yyyy	\" />"+ "</span>"+
					 * "</span>"+ "<button type='button' id = '"+targetAttributeControlObject.getName()
					 * +"DeleteButton'  class='btn flat-border btn-mySmall deleter btn-warning dropdown-toggle' >"+
					...
					+ targetAttributeControlObject.getName()
					+ "TDateSelection').datetimepicker({pickTime: false, defaultDate:\"\"});" +
					// "var picker = $('#start_dateTDateSelection'l).data('datetimepicker');picker.setLocalDate(new
					// Date(Date.UTC(2014, 2, 1, 0, 0));"+
					"});" +
		...
		else if ((targetAttributeControlObject.getDatatype().equalsIgnoreCase("time")))
		{
			...
			htmlString = "<div class=\"marginise  " + targetAttributeControlObject.getName() + "AttributeSelectionRow\""
					...
					"<span class=\"input-group date col-xs-2\" style=\"width: 120px;\" id=\""
					+ targetAttributeControlObject.getName() + "TimeSelection\">" + "<span class=\"input-group-addon\">"
					+ "<span class=\"glyphicon glyphicon-time\"></span>" + "</span>"
					+ "<input type=\"text\" class=\"form-control\" placeholder=\"hh:mm AM	\" />" + "</span>"
					+ "</span>" + "<button type='button' id = '" + targetAttributeControlObject.getName()
					+ "DeleteButton'  class='btn  deleter  btn-mySmall btn-warning dropdown-toggle' >"
					+ "<span class='glyphicon glyphicon-remove'></span>" + "</button>" + "</div>" + "</div>" +
					...
					// "var
					// picker=('#"+targetAttributeControlObject.getName()+"TimeSelection').data('datetimepicker');"+
					// "picker.setDate(null);"+
		...
		// int or big decimal
		{
			htmlString = "<div class=\"marginise   " + targetAttributeControlObject.getName()
					...
					+ "<li><a href=\"#\">=</a></li>" + "<li><a href=\"#\">>=</a></li>" + "<li><a href=\"#\"><=</a></li>"
					+ "<li><a href=\"#\">!=</a></li>" + "</ul>" + "</span>"
					+ "<span class=\"input-group date col-xs-2\" style=\"width: 55px;\" id=\""
					+ targetAttributeControlObject.getName() + "IntDecimalSelection\">" +
					/*
					 * "<span class=\"input-group-addon\">"+ "<span class=\"glyphicon glyphicon-time\"></span>"+
					 * "</span>"+
					 */
					"<input type=\"text\" class=\"form-control\" id=\"" + targetAttributeControlObject.getName()
					+ "Selection\">" + "</span>" + "</span>" + "<button type='button' id = '"
					+ targetAttributeControlObject.getName()
		...
		if (targetAttributeControlObject.getHasOperatorSelection())// !(targetAttributeControlObject.getDatatype().equalsIgnoreCase("Varchar")))//REPLACE
																	// WWITH HAS OP SELECTIOn
		{
			// this means it has operator
			htmlString = htmlString + "\n<script>" + "var " + targetAttributeControlObject.getName()
					+ "SelectedOperator;" + "$(\"." + targetAttributeControlObject.getName()
					+ "OperatorSelection li a\").click(function()" + "{" + "var selText = $(this).text();" + "" + // alert(\"operator
																													// selection\"+selText);
	...
	public void setHasOperatorSelection(String attributeType)
	{
		...
		// %% System.out.println("ZZZZZZZZZZZZZZZZZZZZZZZinside setting has operator selection for attribute"+
		// this.name+" , type="+attributeType);
		if (((attributeType.equalsIgnoreCase("int")) || (attributeType.equalsIgnoreCase("Time"))
				|| (attributeType.equalsIgnoreCase("Date")) || (attributeType.equalsIgnoreCase("Decimal"))))
		{
